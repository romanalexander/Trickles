...
# 208 "/home/ashieh/current/include/net/cminisock.h"
...
# 231 "/home/ashieh/current/include/net/cminisock.h"
...
# 475 "/home/ashieh/current/include/net/cminisock.h"
struct ucontdesc;
...
# 522 "/home/ashieh/current/include/net/cminisock.h"
...
# 33 "/home/ashieh/current/include/net/trickles_state_cache.h"
...
struct sk_buff;
# 256 "/home/ashieh/current/include/net/trickles_packet_helpers.h"
...
# 298 "/home/ashieh/current/include/net/trickles_packet_helpers.h"
...
static inline struct cminisock *copyClientSide_Continuation(struct cminisock *cont, int flags) {
  ...
  struct cminisock *rval = kmem_cache_alloc(clientSideContinuation_cache, (0x20));
# 401 "/home/ashieh/current/include/net/trickles_packet_helpers.h"
  if(rval == ((void *)0)) {
  ...
  for(i=0; i < 2; i++) {
# 419 "/home/ashieh/current/include/net/trickles_packet_helpers.h"
          (rval+i)->ucont_len = 0;
          (rval+i)->ucont_data = ((void *)0);
...
# 453 "/home/ashieh/current/include/net/trickles_packet_helpers.h"
  int i;
# 473 "/home/ashieh/current/include/net/trickles_packet_helpers.h"
  if(cont->list) {
          do { if (!(0)) { printk("kgdb assertion failed: %s\n", "BUG"); show_stack(((void *)0)); breakpoint(); } } while (0);
  }
...
# 498 "/home/ashieh/current/include/net/trickles_packet_helpers.h"
...
# 600 "/home/ashieh/current/include/net/trickles_packet_helpers.h"
...
# 618 "/home/ashieh/current/include/net/trickles_packet_helpers.h"
  printk("Dependency management doesn't work yet\n");
  do { if (!(0)) { printk("kgdb assertion failed: %s\n", "BUG"); show_stack(((void *)0)); breakpoint(); } } while (0);
  return -1;
...
# 647 "/home/ashieh/current/include/net/trickles_packet_helpers.h"
  printk("Dependency management doesn't work yet\n");
  do { if (!(0)) { printk("kgdb assertion failed: %s\n", "BUG"); show_stack(((void *)0)); breakpoint(); } } while (0);
  return ((void *)0);
...
# 664 "/home/ashieh/current/include/net/trickles_packet_helpers.h"
  printk("Dependency management doesn't work yet\n");
  do { if (!(0)) { printk("kgdb assertion failed: %s\n", "BUG"); show_stack(((void *)0)); breakpoint(); } } while (0);
...
# 692 "/home/ashieh/current/include/net/trickles_packet_helpers.h"
  printk("Dependency management doesn't work yet\n");
  do { if (!(0)) { printk("kgdb assertion failed: %s\n", "BUG"); show_stack(((void *)0)); breakpoint(); } } while (0);
  return -1;
...
# 846 "/home/ashieh/current/include/net/trickles_packet_helpers.h"
...
# 981 "/home/ashieh/current/include/net/trickles_packet_helpers.h"
}
...
# 487 "/home/ashieh/current/include/net/trickles.h" 2
# 7 "trickles-int.h" 2
...
# 6 "trickles-userapi.c" 2
# 22 "trickles-userapi.c"
...
void dump_socket_stats(struct sock *sk) {
        ...
        ({ struct tcp_opt *tp = &sk->tp_pinfo.af_tcp; printk("A = %d, D = %d, RTO = %d", tp->t.A, tp->t.D, tp->t.RTO); });
...
void dump_datachunks(char *ptr, int len) {
        ...
        while((char*)(chunk+1) - ptr < len && chunknum < 5) {
                printk("Chunk %d(%d) - %d @ %d\n", chunknum,
                       ...
                       ntohs((chunk)->chunkLen) - sizeof(struct DataChunk),
                       (char*)chunk->data - ptr);
                chunk = ((void*)((char*)(chunk) + ntohs((chunk)->chunkLen)));
                chunknum++;
        }
...
static inline void init_sock(struct cminisock *msk, struct sock *sk) {
        msk->sk = sk;
        msk->sk->dst_cache = ((void *)0);
        msk->sk->protinfo.af_inet.opt = ((void *)0);
        ...
        if (ipv4_config.no_pmtu_disc)
                msk->sk->protinfo.af_inet.pmtudisc = 0;
        else
                msk->sk->protinfo.af_inet.pmtudisc = 1;
...
static volatile void *virt_to_kseg(volatile void *address)
{
        ...
        va=((unsigned long)((unsigned long)address));


        pgd = ((&init_mm)->pgd+((va >> 22) & (1024 -1)));
        ...
        if (!pgd_none(*pgd))
        {
              ...
              if (!(!((*pmd).pmd)))
              {
                  ...
                  ptep = ((pte_t *) ((unsigned long) ((void *)((unsigned long)(((*(pmd)).pmd) & (~((1UL << 12)-1)))+((unsigned long)(0xC0000000))))) + ((va >> 12) & (1024 - 1)));
                  pte = *ptep;

                  if (((pte).pte_low & (0x001 | 0x080)))
                  {

                        ret = (unsigned long)((void *)((unsigned long)(((((mem_map+((unsigned long)(((pte).pte_low >> 12))))) - page_zone((mem_map+((unsigned long)(((pte).pte_low >> 12)))))->zone_mem_map) << 12) + page_zone((mem_map+((unsigned long)(((pte).pte_low >> 12)))))->zone_start_paddr)+((unsigned long)(0xC0000000))));
...
# 214 "trickles-userapi.c"
inline void queue_upcall_msk_prealloc(struct sock *sk, enum cminisock_event_tag tag, struct cminisock *msk) { struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp); switch(tag) { case SYN: case RST: case FIN: case ACK: insert_tail_mb(&tp->cminisock_api_config.cfg.ctl->msk_eventlist, (struct alloc_head *)msk);; break; default: printk("Unsupported tag for preallocation\n"); } } inline void queue_upcall_msk(enum cminisock_event_tag tag, struct cminisock *msk) { msk->tag = tag; switch(tag) { case SYN: case RST: case FIN: case ACK: msk->ctl = ALLOC_READY; if(userapi_pkt_spew) { int i; for(i=0; i < msk->num_packets; i++) { printk("upcall pkts[%d]: %u-%u\n", i, msk->packets[i].seq, msk->packets[i].seq + msk->packets[i].len); } } break; default: printk("queue_upcall_msk (%s): invalid tag\n", "msk"); } };

inline void queue_upcall_pmsk_prealloc(struct sock *sk, enum cminisock_event_tag tag, struct pminisock *msk) { struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp); switch(tag) { case SYN: case RST: case FIN: case ACK: dlist_insert_tail_mb(&tp->cminisock_api_config.cfg.ctl->pmsk_eventlist, (struct list_link *)msk);; break; default: printk("Unsupported tag for preallocation\n"); } } inline void queue_upcall_pmsk(enum cminisock_event_tag tag, struct pminisock *msk) { msk->tag = tag; switch(tag) { case SYN: case RST: case FIN: case ACK: msk->ctl = ALLOC_READY; if(userapi_pkt_spew) { int i; for(i=0; i < msk->num_packets; i++) { printk("upcall pkts[%d]: %u-%u\n", i, msk->packets[i].seq, msk->packets[i].seq + msk->packets[i].len); } } break; default: printk("queue_upcall_msk (%s): invalid tag\n", "pmsk"); } };
...
static void tcp_prequeue_process(struct sock *sk)
{
        ...
        net_statistics[0*2+1].TCPPrequeued += skb_queue_len(&tp->ucopy.prequeue);


        do { ((void)((0)), irq_stat[0].__local_bh_count)++; __asm__ __volatile__("": : :"memory"); } while (0);
        while ((skb = __skb_dequeue(&tp->ucopy.prequeue)) != ((void *)0))
                sk->backlog_rcv(sk, skb);
        do { unsigned int *ptr = &((void)((0)), irq_stat[0].__local_bh_count); __asm__ __volatile__("": : :"memory"); if (!--*ptr) __asm__ __volatile__ ( "cmpl $0, -8(%0);" "jnz 2f;" "1:;" ".subsection 1\n\t" "" ".ifndef " ".text.lock." "tmalloc" "\n\t" ".text.lock." "tmalloc" ":\n\t" ".endif\n\t" "2: pushl %%eax; pushl %%ecx; pushl %%edx;" "call %c1;" "popl %%edx; popl %%ecx; popl %%eax;" "jmp 1b;" ".previous\n\t" : : "r" (ptr), "i" (do_softirq) ); } while (0);
...
void finishTransmitHelper(struct cminisock *msk, struct sk_buff *skbs[],
        ...
        for(i = numDataPackets; i < msk->num_packets; i++) {
                ...
                skb = alloc_skb((128 + 32) + (sizeof(WireTrickleResponse)) + ucontLen, (0x20 | 0x10 | 0x40 | 0x80 | 0x100));
                if(skb == ((void *)0)) {
                        printk("out of memory during finishTransmit\n");
                        return;
                }
                skb_reserve(skb, (128 + 32) + (sizeof(WireTrickleResponse)) + ucontLen);
                skb->csum = 0;
                if((0)) {
                        return;
                }
...
static inline int tiov_handler(struct sock *origSK, struct cminisock *msk, struct tiovec *tiov, int tiovlen) {
        ...
        struct sk_buff *skb_body[(4)];
        struct sk_buff **skbs, *skb;
        if(msk->num_packets > (4)) {
                skbs = kmalloc(sizeof(struct sk_buff *) * msk->num_packets, ( 0x10 | 0x40 | 0x80 | 0x100));
        } else {
                if (!(msk->num_packets >= 0)) { printk("KERNEL: assertion (" "msk->num_packets >= 0" ") failed at " "trickles-userapi.c" "(%d)\n", 517); };
                skbs = skb_body;
        }
        int maxLen = 0;
        ...
        for(i=0; i < msk->num_packets; i++) {
                ...
                pkt_remain = pkts[i].len = ({ typeof(pkts[i].len) Z = (typeof(pkts[i].len))(totallen); ({ const typeof(pkts[i].len) _x = (pkts[i].len); const typeof(Z) _y = (Z); (void) (&_x == &_y); _x < _y ? _x : _y; }); });
                skb = skbs[i] = recycle(origSK);
                if(skb == ((void *)0)) {
                        skb = skbs[i] = alloc_skb((128 + 32) + (sizeof(WireTrickleResponse)) + pkts[i].ucontLen + pkts[i].len, (0x20 | 0x10 | 0x40 | 0x80 | 0x100));
                }
                numDataPackets = i+1;
                if(skb == ((void *)0)) {
                ...
                skb_reserve(skb, (128 + 32) + (sizeof(WireTrickleResponse)) + pkts[i].ucontLen);

                skb->csum = 0;
                skb->ip_summed = 1;
                while(tiov_pos < tiovlen) {
                        int amt = ({ typeof(pkt_remain) Z = (typeof(pkt_remain))(tiov[tiov_pos].iov_len - tiov_offs); ({ const typeof(pkt_remain) _x = (pkt_remain); const typeof(Z) _y = (Z); (void) (&_x == &_y); _x < _y ? _x : _y; }); });
                        int err = 0;
                ...
                if (!(pkt_remain == 0)) { printk("KERNEL: assertion (" "pkt_remain == 0" ") failed at " "trickles-userapi.c" "(%d)\n", 600); };
        ...
        finishTransmitHelper(msk,skbs,numDataPackets,((void *)0),0,0);
...
void trickles_close(struct sock *sk, long timeout) {
        ...
        while((skb=__skb_dequeue(&sk->receive_queue))!=((void *)0)) {
                u32 len = ((struct tcp_skb_cb *)&((skb)->cb[0]))->end_seq - ((struct tcp_skb_cb *)&((skb)->cb[0]))->seq - skb->h.th->fin;
                data_was_unread += len;
                __kfree_skb(skb);
        }
...
int trickles_sendmsg(struct sock *sk, struct msghdr *msg, int size) {
        ...
        if(sizeof(*cmd) != msg->msg_namelen || (int)cmd->magic != (0xbaadd33d)) {
                return -22;
        }
        ...
        if(!((tp)->cminisock_api_config.cfg.mmap_base != ((void *)0))) {
                return -22;
        }
        if(cmd->cmd == POLL) {
                int res;
        ...
        (msk)->pmsk = ((void *)0); msk_clear_fields((msk)); free_trickles_msk_finish((sk),(msk));;;
...
static int trickles_poll(struct sock *sk) {
        int err = 0;
# 856 "trickles-userapi.c"
        struct socket sock;
        sock.sk = sk;
        if((trickles_sock_poll_impl(((void *)0), &sock, ((void *)0)) == 0)) {
...
int trickles_sock_poll_impl(struct file * file, struct socket *sock, poll_table *wait) {
    ...
    int mask = 0;
    if(((tp)->cminisock_api_config.cfg.mmap_base != ((void *)0))) {
            int poll_value = ((&tp->cminisock_api_config.cfg.ctl->update_since_poll)->counter);
            if((!(tp->trickles_opt & 0x80) && !dlist_empty(&tp->cminisock_api_config.cfg.ctl->pmsk_eventlist)) ||
               ((tp->trickles_opt & 0x80) &&
                poll_value > 0 &&
...
static struct page * trickles_vma_nopage(struct vm_area_struct * area, unsigned long address, int unused) {
        ...
        if(api_config == ((void *)0)) return ((void *)0);
        ...
        page = (mem_map + (((unsigned long)(virt_to_kseg((char *)base + offset))-((unsigned long)(0xC0000000))) >> 12));
        if(page)
...
int cminisock_config_pipe_impl(struct sock *sk, char *optdata, int optlen, int direction) {
        ...
        if(direction == 0) {
                ...
                error = (__builtin_constant_p(sizeof(new_config)) ? __constant_copy_from_user((&new_config),(optdata),(sizeof(new_config))) : __generic_copy_from_user((&new_config),(optdata),(sizeof(new_config))));
                if(error) {
                ...
                if((mmap_len = new_config.mmap_len) > ((1<<16) * (1UL << 12))) {
                ...
                if(new_config.mmap_base == ((void *)0)) {
                ...
                new_config.ctl = ctl;
                if (!(sizeof(*ctl) + sizeof(struct tq_struct) < (1UL << 12))) { printk("KERNEL: assertion (" "sizeof(*ctl) + sizeof(struct work_struct) < PAGE_SIZE" ") failed at " "trickles-userapi.c" "(%d)\n", 991); };
                ctl->ro_base = new_config.mmap_base;
                ctl->ro_len = (((mmap_len / 4)+(1UL << 12)-1)&(~((1UL << 12)-1)));
                ctl->ro_offs = (char*)ctl->ro_base - (char*)new_config.mmap_base;
                ctl->rw_base = (__u8*)ctl->ro_base + ctl->ro_len;
                ctl->rw_len = (((mmap_len - ((char*)ctl->rw_base - (char*)new_config.mmap_base))+(1UL << 12)-1)&(~((1UL << 12)-1))) - (1UL << 12);
                ctl->rw_offs = (char*)ctl->rw_base - (char*)new_config.mmap_base;
                (((&ctl->update_since_poll)->counter) = (0));
                do { do { (&((struct tq_struct *)(ctl+1))->list)->next = (&((struct tq_struct *)(ctl+1))->list); (&((struct tq_struct *)(ctl+1))->list)->prev = (&((struct tq_struct *)(ctl+1))->list); } while (0); ((struct tq_struct *)(ctl+1))->sync = 0; do { (((struct tq_struct *)(ctl+1)))->routine = (vfree_helper); (((struct tq_struct *)(ctl+1)))->data = (new_config.mmap_base); } while (0); } while (0);
                if (!(ctl->rw_len > 0)) { printk("KERNEL: assertion (" "ctl->rw_len > 0" ") failed at " "trickles-userapi.c" "(%d)\n", 1000); };
                if (!(ctl->ro_len > 0)) { printk("KERNEL: assertion (" "ctl->ro_len > 0" ") failed at " "trickles-userapi.c" "(%d)\n", 1001); };
                if (!((char*)ctl->ro_base < (char*)new_config.mmap_base + mmap_len)) { printk("KERNEL: assertion (" "(char*)ctl->ro_base < (char*)new_config.mmap_base + mmap_len" ") failed at " "trickles-userapi.c" "(%d)\n", 1002); };

                init_head(&ctl->msk_eventlist);

                tp->cminisock_api_config.cfg = new_config;
                new_config.ctl = ((void *)0);


                head = &tp->cminisock_api_config.msk_freelist;
                init_head(head);

                msk = (struct cminisock *)((char*)ctl->ro_base + (1UL << 12));
                ctl->minisock_base = msk;
                ctl->minisock_offs = (char*)ctl->minisock_base - (char*)new_config.mmap_base;

                minisock_len = ((ctl->ro_len) / 4);
                ctl->minisock_limit = (char*)ctl->minisock_base + minisock_len;
                if (!(minisock_len > 0)) { printk("KERNEL: assertion (" "minisock_len > 0" ") failed at " "trickles-userapi.c" "(%d)\n", 1019); };
                while((char*)(msk + 1) <= (char*)ctl->minisock_limit) {
                        (__builtin_constant_p(0x3a) ? (__builtin_constant_p((sizeof(*msk))) ? __constant_c_and_count_memset(((msk)),((0x01010101UL*(unsigned char)(0x3a))),((sizeof(*msk)))) : __constant_c_memset(((msk)),((0x01010101UL*(unsigned char)(0x3a))),((sizeof(*msk))))) : (__builtin_constant_p((sizeof(*msk))) ? __memset_generic((((msk))),(((0x3a))),(((sizeof(*msk))))) : __memset_generic(((msk)),((0x3a)),((sizeof(*msk))))));
                        msk->ctl = ALLOC_FREE;
                        msk->prev = msk->next = ((void *)0);
                        msk->list = ((void *)0);
                        init_minisock(msk);
                ...
                int pminisock_len = ((ctl->ro_len) / 4);
                ctl->pminisock_base = ((typeof(ctl->minisock_limit))(char*)(ctl->minisock_limit) + (1 - ((int)ctl->minisock_limit) % 1) % 1);
                ...
                while((char*)(pmsk+1) <= (char*)ctl->pminisock_limit) {
                        (__builtin_constant_p(0x3a) ? (__builtin_constant_p((sizeof(*pmsk))) ? __constant_c_and_count_memset(((pmsk)),((0x01010101UL*(unsigned char)(0x3a))),((sizeof(*pmsk)))) : __constant_c_memset(((pmsk)),((0x01010101UL*(unsigned char)(0x3a))),((sizeof(*pmsk))))) : (__builtin_constant_p((sizeof(*pmsk))) ? __memset_generic((((pmsk))),(((0x3a))),(((sizeof(*pmsk))))) : __memset_generic(((pmsk)),((0x3a)),((sizeof(*pmsk))))));
                        pmsk->prev = pmsk->next = ((void *)0);
                        pmsk->ctl = ALLOC_FREE;
                        init_pminisock(pmsk);
                ...
                ctl->heap_base = ((typeof(ctl->pminisock_limit))(char*)(ctl->pminisock_limit) + (1 - ((int)ctl->pminisock_limit) % 1) % 1);
                tp->t.heapbytesize = ctl->ro_len - pminisock_len;

                error = (__builtin_constant_p(sizeof(new_config)) ? __constant_copy_to_user((optdata),(&new_config),(sizeof(new_config))) : __generic_copy_to_user((optdata),(&new_config),(sizeof(new_config))));
                if(error) {
                ...
                if(generateHMAC) {
                        ...
                        __u8 hmac_key[(16)+1] = "\00\01\02\03\04\05\06\07\010\011\012\013\014\015\016\017\020\021\022\023\024";



                        tp->t.hmacCTX = kmalloc(sizeof(*tp->t.hmacCTX), (0x20 | 0x10 | 0x40 | 0x80 | 0x100));
                        if(tp->t.hmacCTX == ((void *)0)) {
                                printk("hmac kmalloc error\n");
                                error = -12;
                                goto out_dealloc;
                        }
                        hmac_setup(tp->t.hmacCTX, hmac_key, (16));
                ...
                if(generateNonces) {
                        ...
                        __u8 nonce_key[(16)+1] = "\00\01\02\03\04\05\06\07\010\011\012\013\014\015\016\017\020\021\022\023\024";

                        tp->t.nonceCTX = kmalloc(sizeof(*tp->t.nonceCTX), (0x20 | 0x10 | 0x40 | 0x80 | 0x100));
                        if(tp->t.nonceCTX == ((void *)0)) {
                                printk("nonce kmalloc error\n");
                                error = -12;
                                goto out_dealloc;
                        }
                        aes_encrypt_key(nonce_key, (16), tp->t.nonceCTX);
                }

                return 0;
        } else {
                int len;
                if(({ int __ret_gu,__val_gu; switch(sizeof (*((int*)optlen))) { case 1: __asm__ __volatile__("call __get_user_" "1" :"=a" (__ret_gu),"=d" (__val_gu) :"0" ((int*)optlen)); break; case 2: __asm__ __volatile__("call __get_user_" "2" :"=a" (__ret_gu),"=d" (__val_gu) :"0" ((int*)optlen)); break; case 4: __asm__ __volatile__("call __get_user_" "4" :"=a" (__ret_gu),"=d" (__val_gu) :"0" ((int*)optlen)); break; default: __asm__ __volatile__("call __get_user_" "X" :"=a" (__ret_gu),"=d" (__val_gu) :"0" ((int*)optlen)); break; } (len) = (__typeof__(*((int*)optlen)))__val_gu; __ret_gu; })) {
                        printk("pipe parameter readout: get optlen fault\n");
                        return -14;
                }
                error = (__builtin_constant_p(sizeof(tp->cminisock_api_config.cfg)) ? __constant_copy_to_user((optdata),(&tp->cminisock_api_config.cfg),(sizeof(tp->cminisock_api_config.cfg))) : __generic_copy_to_user((optdata),(&tp->cminisock_api_config.cfg),(sizeof(tp->cminisock_api_config.cfg))));
                if(error) {
                        printk("pipe parameter readout: copy out fault\n");
                        return -22;
                }
                if(({ long __pu_err = -14; __typeof__(*(((int*)optlen))) *__pu_addr = (((int*)optlen)); if ((({ unsigned long flag,sum; asm("addl %3,%1 ; sbbl %0,%0; cmpl %1,%4; sbbl $0,%0" :"=&r" (flag), "=r" (sum) :"1" (__pu_addr),"g" ((int)(sizeof(*((int*)optlen)))),"g" (get_current()->addr_limit.seg)); flag; }) == 0)) do { __pu_err = 0; switch ((sizeof(*((int*)optlen)))) { case 1: __asm__ __volatile__( "1:	mov""b"" %""b""1,%2\n" "2:\n" ".section .fixup,\"ax\"\n" "3:	movl %3,%0\n" "	jmp 2b\n" ".previous\n" ".section __ex_table,\"a\"\n" "	.align 4\n" "	.long 1b,3b\n" ".previous" : "=r"(__pu_err) : "iq" (((__typeof__(*((int*)optlen)))(len))), "m"((*(struct __large_struct *)(__pu_addr))), "i"(-14), "0"(__pu_err)); break; case 2: __asm__ __volatile__( "1:	mov""w"" %""w""1,%2\n" "2:\n" ".section .fixup,\"ax\"\n" "3:	movl %3,%0\n" "	jmp 2b\n" ".previous\n" ".section __ex_table,\"a\"\n" "	.align 4\n" "	.long 1b,3b\n" ".previous" : "=r"(__pu_err) : "ir" (((__typeof__(*((int*)optlen)))(len))), "m"((*(struct __large_struct *)(__pu_addr))), "i"(-14), "0"(__pu_err)); break; case 4: __asm__ __volatile__( "1:	mov""l"" %""""1,%2\n" "2:\n" ".section .fixup,\"ax\"\n" "3:	movl %3,%0\n" "	jmp 2b\n" ".previous\n" ".section __ex_table,\"a\"\n" "	.align 4\n" "	.long 1b,3b\n" ".previous" : "=r"(__pu_err) : "ir" (((__typeof__(*((int*)optlen)))(len))), "m"((*(struct __large_struct *)(__pu_addr))), "i"(-14), "0"(__pu_err)); break; case 8: __asm__ __volatile__( "1:	movl %%eax,0(%2)\n" "2:	movl %%edx,4(%2)\n" "3:\n" ".section .fixup,\"ax\"\n" "4:	movl %3,%0\n" "	jmp 3b\n" ".previous\n" ".section __ex_table,\"a\"\n" "	.align 4\n" "	.long 1b,4b\n" "	.long 2b,4b\n" ".previous" : "=r"(__pu_err) : "A" (((__typeof__(*((int*)optlen)))(len))), "r" (__pu_addr), "i"(-14), "0"(__pu_err)); break; default: __put_user_bad(); } } while (0); __pu_err; })) {
...
int trickles_sendv_impl(int fd, struct cminisock *msk, struct tiovec *user_tiov, int tiovlen) {
        ...
        err = -22;
        if (tiovlen < 0 || tiovlen > 1024) {
                goto out;
        }
        err = -12;
        tiov_size = sizeof(struct tiovec) * tiovlen;
        if(tiov_size > 0) {
                tiov = kmalloc(tiov_size, (0x20 | 0x10 | 0x40 | 0x80 | 0x100));
                if(tiov == ((void *)0)) {
                        goto out;
                }
                err = -14;
                if((__builtin_constant_p(tiov_size) ? __constant_copy_from_user((tiov),(user_tiov),(tiov_size)) : __generic_copy_from_user((tiov),(user_tiov),(tiov_size)))) {
                        goto out_freeiov;
                }
        } else {
                tiov = ((void *)0);
...
static ssize_t file_insertpages(struct sk_buff *skb, int in_fd, loff_t offset, size_t count)
{
        ...
        if (!in_inode->i_mapping->a_ops->readpage)
                goto fput_in;
        retval = locks_verify_area(1, in_inode, in_file, in_file->f_pos, count);
        if (retval)
                goto fput_in;
...
static inline int fiov_handler(struct sock *origSK, struct cminisock *msk, struct fiovec *fiov, int fiovlen) {
        ...
        int rval = 0;
        struct sk_buff **skbs =
                kmalloc(sizeof(struct sk_buff *) * msk->num_packets, ( 0x10 | 0x40 | 0x80 | 0x100)),
                *skb;
        ...
        for(i=0; i < msk->num_packets; i++) {
                ...
                while(fiov_pos < fiovlen) {
                        int amt = ({ typeof(pkt_remain) Z = (typeof(pkt_remain))(fiov[fiov_pos].len - fiov_offs); ({ const typeof(pkt_remain) _x = (pkt_remain); const typeof(Z) _y = (Z); (void) (&_x == &_y); _x < _y ? _x : _y; }); });
                        int err = 0;
...
int trickles_sendfilev_impl(int fd, struct cminisock *msk, struct fiovec *user_fiov, int fiovlen) {
        ...
        }
        {
                ...
                if(fiov_size > 0) {
                        fiov = kmalloc(fiov_size, (0x20 | 0x10 | 0x40 | 0x80 | 0x100));
                        if(fiov == ((void *)0)) {
                                goto out;
                        }
                        err = -14;
                        if((__builtin_constant_p(fiov_size) ? __constant_copy_from_user((fiov),(user_fiov),(fiov_size)) : __generic_copy_from_user((fiov),(user_fiov),(fiov_size)))) {
                                goto out_freefiov;
                        }
                } else {
                        fiov = ((void *)0);
...
# 1565 "trickles-userapi.c"
        ...
        if(iov.iov_len == -1) {
                ...
                if(msk->ctl != ALLOC_HALFFREE) { msk->ctl = ALLOC_HALFFREE; err = 0; } else { printk("already half freed\n"); err = -22; }
# 1595 "trickles-userapi.c"
...
# 1666 "trickles-userapi.c"
        ...
        if(ucont_len > 0) {
                buf = kmalloc(ucont_len, (0x20 | 0x10 | 0x40 | 0x80 | 0x100));
                if(buf == ((void *)0)) {
                ...
                if((__builtin_constant_p(ucont_len) ? __constant_copy_from_user((buf),(user_ucont),(ucont_len)) : __generic_copy_from_user((buf),(user_ucont),(ucont_len)))) {
        ...
        if (!(pkt_num < pmsk->num_packets)) { printk("KERNEL: assertion (" "pkt_num < pmsk->num_packets" ") failed at " "trickles-userapi.c" "(%d)\n", 1693); };
...
static int wait_for_write_memory(struct sock * sk, long *timeo)
{
        ...
        wait_queue_t wait = { task: get_current(), task_list: { ((void *)0), ((void *)0) }, };

        if (trickles_write_memory_free(sk))
                current_timeo = vm_wait = (net_random()%(100/5))+2;

        add_wait_queue(sk->sleep, &wait);
        for (;;) {
                set_bit(0, &sk->socket->flags);

                do { get_current()->state = (1); __asm__ __volatile__("": : :"memory"); } while (0);

                if (sk->err || (sk->shutdown & 2))
                        goto do_error;
                if (!*timeo)
                        goto do_nonblock;
                if (signal_pending(get_current()))
                        goto do_interrupted;
                clear_bit(0, &sk->socket->flags);
                if (trickles_write_memory_free(sk) && !vm_wait)
                        break;

                set_bit(2, &sk->socket->flags);
                do { do { do { ((void)((0)), irq_stat[0].__local_bh_count)++; __asm__ __volatile__("": : :"memory"); } while (0); (void)(&((sk)->lock.slock)); } while (0); if ((sk)->backlog.tail != ((void *)0)) __release_sock(sk); (sk)->lock.users = 0; if (waitqueue_active(&((sk)->lock.wq))) __wake_up((&((sk)->lock.wq)),2 | 1, 1); do { do { } while(0); do { unsigned int *ptr = &((void)((0)), irq_stat[0].__local_bh_count); __asm__ __volatile__("": : :"memory"); if (!--*ptr) __asm__ __volatile__ ( "cmpl $0, -8(%0);" "jnz 2f;" "1:;" ".subsection 1\n\t" "" ".ifndef " ".text.lock." "tmalloc" "\n\t" ".text.lock." "tmalloc" ":\n\t" ".endif\n\t" "2: pushl %%eax; pushl %%ecx; pushl %%edx;" "call %c1;" "popl %%edx; popl %%ecx; popl %%eax;" "jmp 1b;" ".previous\n\t" : : "r" (ptr), "i" (do_softirq) ); } while (0); } while (0); } while(0);
                if (!trickles_write_memory_free(sk) || vm_wait)
...
int trickles_client_sendmsg(struct sock *sk, struct msghdr *msg, int size) {
                ...
                while(iov_position < iov[i].iov_len) {
                        ...
                        if(skb_spaceleft == 0) {
                                if(skb != ((void *)0)) {
                                        ...
                                        ((struct tcp_skb_cb *)&((skb)->cb[0]))->end_seq = tp->t.write_seq;
                                        __skb_queue_tail(&tp->t.requestBytes, skb);
                                }
                                if(!((sk)->tp_pinfo.af_tcp.t.write_seq < (sk)->tp_pinfo.af_tcp.t.snd_una + (sk)->sndbuf)) {
                                        long timeout;
                                sleep:
                                        timeout = 100/10;
                                        wait_for_write_memory(sk, &timeout);
                                }
                                if (!(tp->t.write_seq - tp->t.snd_una <= sk->sndbuf)) { printk("KERNEL: assertion (" "tp->t.write_seq - tp->t.snd_una <= sk->sndbuf" ") failed at " "trickles-userapi.c" "(%d)\n", 1824); };
                                skb_spaceleft = ({ typeof(sk->sndbuf - (tp->t.write_seq - tp->t.snd_una)) Z = (typeof(sk->sndbuf - (tp->t.write_seq - tp->t.snd_una)))(totalLen - position); ({ const typeof(sk->sndbuf - (tp->t.write_seq - tp->t.snd_una)) _x = (sk->sndbuf - (tp->t.write_seq - tp->t.snd_una)); const typeof(Z) _y = (Z); (void) (&_x == &_y); _x < _y ? _x : _y; }); });

                                skb = alloc_skb(skb_spaceleft, (0x20));
                                if(skb == ((void *)0)) {
                                        printk("Trickles_client_sendmsg: out of memory\n");
                                        goto sleep;
                                }
                                ((struct tcp_skb_cb *)&((skb)->cb[0]))->seq = tp->t.write_seq;
                        }
                        copyLen = ({ typeof(iov[i].iov_len - iov_position) Z = (typeof(iov[i].iov_len - iov_position))(skb_spaceleft); ({ const typeof(iov[i].iov_len - iov_position) _x = (iov[i].iov_len - iov_position); const typeof(Z) _y = (Z); (void) (&_x == &_y); _x < _y ? _x : _y; }); });
                        buf = skb_put(skb, copyLen);
                        if((__builtin_constant_p(copyLen) ? __constant_copy_from_user((buf),(iov[i].iov_base + iov_position),(copyLen)) : __generic_copy_from_user((buf),(iov[i].iov_base + iov_position),(copyLen)))) {
        ...
        if (!(skb_spaceleft == 0)) { printk("KERNEL: assertion (" "skb_spaceleft == 0" ") failed at " "trickles-userapi.c" "(%d)\n", 1846); };
        if(skb != ((void *)0)) {
...
void trickles_syn_piggyback_impl(struct sock *sk, struct sk_buff *skb) {
        ...
        if(sendBufLen > 0) {
                int copyLen =
                        ({ typeof(sendBufLen) Z = (typeof(sendBufLen))((sysctl_trickles_mss) - (128 + 32)); ({ const typeof(sendBufLen) _x = (sendBufLen); const typeof(Z) _y = (Z); (void) (&_x == &_y); _x < _y ? _x : _y; }); });
                struct sk_buff *input_skb = tp->t.requestBytes.next;
                int csum = 0;
                while(copyLen > 0) {
                        if (!((void*)input_skb != (void*)&tp->t.requestBytes)) { printk("KERNEL: assertion (" "(void*)input_skb != (void*)&tp->t.requestBytes" ") failed at " "trickles-userapi.c" "(%d)\n", 1902); };
                        unsigned pieceLen = ({ typeof(input_skb->len) Z = (typeof(input_skb->len))(copyLen); ({ const typeof(input_skb->len) _x = (input_skb->len); const typeof(Z) _y = (Z); (void) (&_x == &_y); _x < _y ? _x : _y; }); });
...
int trickles_client_recvmsg(struct sock *sk, struct msghdr *msg,
        ...
        if(!(tp->trickles_opt & 0x10)) {
                ...
                tp->trickles_opt |= 0x10;
        }

        if(!(tp->trickles_opt & 0x40)) {
        ...
        while(1) {
                if(tp->rcv_nxt - tp->copied_seq >= target) {
                        result = ({ typeof(len) Z = (typeof(len))(tp->rcv_nxt - tp->copied_seq); ({ const typeof(len) _x = (len); const typeof(Z) _y = (Z); (void) (&_x == &_y); _x < _y ? _x : _y; }); });
                        tp->copied_seq += result;
                        cleanup_rbuf(sk, result);
                        break;
                } else if(timeo == 0) {

                        result = ({ typeof(target) Z = (typeof(target))(tp->rcv_nxt - tp->copied_seq); ({ const typeof(target) _x = (target); const typeof(Z) _y = (Z); (void) (&_x == &_y); _x < _y ? _x : _y; }); });
                        tp->copied_seq += result;
                        cleanup_rbuf(sk, result);
                        break;
                } else {
...
int trickles_sendbulk_impl(int fd, struct mskdesc *user_descbuf, int descbuf_len) {
        ...
        descbuf = kmalloc(descbuf_len, (0x20 | 0x10 | 0x40 | 0x80 | 0x100));
        if(descbuf == ((void *)0)) {
                printk("trickles_sendbulk_impl: user desc buf out of memory\n");
                goto out_put;
        }
        if((err = (__builtin_constant_p(descbuf_len) ? __constant_copy_from_user((descbuf),(user_descbuf),(descbuf_len)) : __generic_copy_from_user((descbuf),(user_descbuf),(descbuf_len))))) {
        ...
        while(curr_desc < (struct mskdesc *)tail) {
                ...
                if(!({ struct tcp_opt *_tp = &((sk)->tp_pinfo.af_tcp); (((((char*)((msk)) - (char*)((_tp)->cminisock_api_config.cfg.ctl->minisock_base)) % sizeof(struct cminisock)) == 0) && (((void*)((struct cminisock *)((msk)))) >= ((void*)((_tp)->cminisock_api_config.cfg.ctl->minisock_base)) && ((void*)((struct cminisock *)((msk)) + 1)) <= ((_tp)->cminisock_api_config.cfg.ctl->minisock_limit))) && (((msk))->ctl == ALLOC_READY || ((msk))->ctl == ALLOC_HALFFREE); })) {
                        printk("trickles_sendbulk_impl: bad minisocket %d %d @ %d\n",
                               !(((((char*)(msk) - (char*)((tp)->cminisock_api_config.cfg.ctl->minisock_base)) % sizeof(struct cminisock)) == 0) && (((void*)((struct cminisock *)(msk))) >= ((void*)((tp)->cminisock_api_config.cfg.ctl->minisock_base)) && ((void*)((struct cminisock *)(msk) + 1)) <= ((tp)->cminisock_api_config.cfg.ctl->minisock_limit))), !(((msk)->ctl == ALLOC_READY || (msk)->ctl == ALLOC_HALFFREE)),
                               descnum);
                        err = -22;
                        goto out_release;
                }
                ...
                curr_desc = ({ (struct mskdesc*)(&((struct mskdesc *)curr_desc)->tiov[((struct mskdesc *)curr_desc)->tiov_num]); });
                descnum++;
                       ...
                       int *eof, void *data)
{
    ...
    do { ((void)((0)), irq_stat[0].__local_bh_count)++; __asm__ __volatile__("": : :"memory"); } while (0);
    for (logEntry = (typeof(logEntry))(&tricklesProcLogHead)->next; (logEntry != (typeof(logEntry))(&tricklesProcLogHead)); logEntry=(typeof(logEntry))logEntry->next) {
            ...
            for(i=logEntry->returnedEvents; i < logEntry->size; i++) {
                    ...
                    if(done) {
                            ...
                            if (!(pos == count)) { printk("KERNEL: assertion (" "pos == count" ") failed at " "trickles-userapi.c" "(%d)\n", 2239); };
                            goto done;
                    }
            }
...
int bb_write(struct BoundedBuffer *bb, const char *data, int len) {
        ...
        int copyLen;
        bb->curPos = ({ typeof(prevPos+len) Z = (typeof(prevPos+len))(bb->len); ({ const typeof(prevPos+len) _x = (prevPos+len); const typeof(Z) _y = (Z); (void) (&_x == &_y); _x < _y ? _x : _y; }); });
        copyLen = bb->curPos - prevPos;

        (__builtin_constant_p(copyLen) ? __constant_memcpy((bb->dest + prevPos),(data),(copyLen)) : __memcpy((bb->dest + prevPos),(data),(copyLen)));
        return copyLen;
}

spinlock_t cwndLogLock = (spinlock_t) { };

void trickles_logCwnd_impl(enum LogCwndType type, int addr, int port, int seq, int ack_seq,
                           int startCwnd, int effCwnd, int ssthresh,
                           int rtt, int srtt) {
# 2328 "trickles-userapi.c"
}
                            ...
                            int *eof, void *data)
{
    ...
    for (logEntry = (typeof(logEntry))(&tricklesCwndProcLogHead)->next; (logEntry != (typeof(logEntry))(&tricklesCwndProcLogHead)); logEntry=(typeof(logEntry))logEntry->next) {
            if(1) {
                    char tempbuf[1024];
...
# 1 "array.h" 1
# 2443 "trickles-userapi.c" 2
# 1 "bounded_buffer.h" 1
...
void *lbb_append(struct linear_bounded_buffer *lbb, void *data, int len) {
        ...
        (__builtin_constant_p(len) ? __constant_copy_to_user_nocheck((dest),(data),(len)) : __generic_copy_to_user_nocheck((dest),(data),(len)));
        return dest;
}
# 2444 "trickles-userapi.c" 2
...
static inline void packetPatchGenerator(const void * const context, int num, struct patch_desc *patch) {
        patch->offset = num * sizeof(struct cminisock_packet) +
                (((char *)&((struct cminisock_packet*)0)->ucontData) - (char*)((void *)0));
        patch->width = (sizeof(((struct cminisock_packet*)0)->ucontData));
}
...
int defer(struct DeferralContext *ctx, int value) {
        while(ctx->next != ((void *)0)) {
                if (!(ctx->numDeferrals == (16))) { printk("KERNEL: assertion (" "ctx->numDeferrals == MAX_NUM_DEFERRALS_PER_CONTEXT" ") failed at " "trickles-userapi.c" "(%d)\n", 2543); };
                ctx = ctx->next;
        }
        if(ctx->numDeferrals == (16)) {
                ctx->next = kmalloc(sizeof(struct DeferralContext), (0x20 | 0x10 | 0x40 | 0x80 | 0x100));
                if(ctx->next == ((void *)0)) {
...
void apply_deferrals(struct HMAC_CTX *hmac_ctx, struct DeferralContext *deferral_ctx) {
        do { if (!(0)) { printk("kgdb assertion failed: %s\n", "BUG"); show_stack(((void *)0)); breakpoint(); } } while (0);
# 2575 "trickles-userapi.c"
}




void *(* const tmalloc_helperout)(struct sock *sk, int len) = ((void *)0);
void *(* const kmalloc_helperout)(struct sock *sk, int len) = ((void *)0);
...
int trickles_extract_events_impl(int fd, struct extract_mskdesc_in *descbuf, int descbuf_len, struct msk_collection *dest, int *destLen) {
        ...
        if(!(((sk)->tp_pinfo.af_tcp.trickles_opt & 0x1) && ((sk)->state == TCP_LISTEN))) {
        ...
        if(!(({ unsigned long flag,sum; asm("addl %3,%1 ; sbbl %0,%0; cmpl %1,%4; sbbl $0,%0" :"=&r" (flag), "=r" (sum) :"1" (descbuf),"g" ((int)(descbuf_len)),"g" (get_current()->addr_limit.seg)); flag; }) == 0)) {
                printk("descbuf not readable\n");
                err = -14;
                goto out_put;
        }
        if(!(({ unsigned long flag,sum; asm("addl %3,%1 ; sbbl %0,%0; cmpl %1,%4; sbbl $0,%0" :"=&r" (flag), "=r" (sum) :"1" (dest),"g" ((int)(*destLen)),"g" (get_current()->addr_limit.seg)); flag; }) == 0)) {
        ...
        *destLen = 0;
        ...
        while((char*)currentInputDesc - (char*) descbuf < descbuf_len) {
                ...
                (__builtin_constant_p(sizeof(mskdesc)) ? __constant_copy_from_user_nocheck((&mskdesc),(currentInputDesc),(sizeof(mskdesc))) : __generic_copy_from_user_nocheck((&mskdesc),(currentInputDesc),(sizeof(mskdesc))));
                ...
                if(mskdesc.operation & (0x1)) {
                        ...
                        if((err = convertMSK_out(sk, &hmac_ctx, msk, &lbb, &numCopies)) != 0) {
                                ...
                                if (!(err == (-2))) { printk("KERNEL: assertion (" "err == MSK_ERROR" ") failed at " "trickles-userapi.c" "(%d)\n", 2894); };
                                if(err == (-2)) {
                                        goto out_put;
                                }
                        }
                ...
                if(mskdesc.operation & (0x2)) {
                        free_trickles_msk(sk,msk);
                        msk->ctl = ALLOC_HALFFREE;
                }
                if(mskdesc.operation & (0x4)) {
                ...
                currentInputDesc = ((currentInputDesc)+1);
        }
 ...
 truncated_output:
        ...
        (__builtin_constant_p(16) ? __constant_copy_to_user_nocheck((outputCollection->hmac),(hmac),(16)) : __generic_copy_to_user_nocheck((outputCollection->hmac),(hmac),(16)));
        ...
        if (!(lbb_get_offset(&lbb) == 0)) { printk("KERNEL: assertion (" "lbb_get_offset(&lbb) == 0" ") failed at " "trickles-userapi.c" "(%d)\n", 2936); };
        struct msk_collection *inputCollection = (struct msk_collection *)lbb_reserve(&lbb, sizeof(struct msk_collection));
        do { if((inputCollection) != (outputCollection)) printk("inputCollection" " = %d, " "outputCollection" " = %d\n", (int)(inputCollection), (int)(outputCollection)); else ; } while(0);
        ...
        int memcmp_result = __builtin_memcmp(test_hmac, hmac, 16);
        do { if((memcmp_result) != (0)) printk("memcmp_result" " = %d, " "0" " = %d\n", (int)(memcmp_result), (int)(0)); else ; } while(0);
# 2965 "trickles-userapi.c"
        if((test_numIn = extract_MSKs_from_lbb(sk, &new_msk_list, &test_hmac_ctx, &lbb)) < 0) {
                printk("test_numIn < 0: %d\n", test_numIn);
        } else {
                do { if((test_numIn) != (numCopies)) printk("test_numIn" " = %d, " "numCopies" " = %d\n", (int)(test_numIn), (int)(numCopies)); else ; } while(0);
                do { if((new_msk_list.len) != (test_numIn)) printk("new_msk_list.len" " = %d, " "test_numIn" " = %d\n", (int)(new_msk_list.len), (int)(test_numIn)); else ; } while(0);
        }
        ...
        if(new_msk_list.len > 0) {
                while(new_msk_list.len > 0) {
                        ...
                        if (!(toFree != (struct cminisock*)&new_msk_list)) { printk("KERNEL: assertion (" "toFree != (struct cminisock*)&new_msk_list" ") failed at " "trickles-userapi.c" "(%d)\n", 2976); };
...
int trickles_install_events_impl(int fd, struct msk_collection *descbuf, int descbuf_len) {
        ...
        while(inputPosition < (char*)descbuf + descbuf_len) {
                ...
                char input_hmac[16];
                (__builtin_constant_p(16) ? __constant_copy_from_user_nocheck((input_hmac),(inputCollection->hmac),(16)) : __generic_copy_from_user_nocheck((input_hmac),(inputCollection->hmac),(16)));
                ...
                if(__builtin_memcmp(hmac, input_hmac, 16) != 0) {
                ...
                if (!(inputPosition == lbb_get_pos(&lbb))) { printk("KERNEL: assertion (" "inputPosition == lbb_get_pos(&lbb)" ") failed at " "trickles-userapi.c" "(%d)\n", 3109); };
                totalNumIn += numIn;
        }
        ...
        while(new_msk_list.len > 0) {
                ...
                if (!(relink != (struct cminisock*)&new_msk_list)) { printk("KERNEL: assertion (" "relink != (struct cminisock*)&new_msk_list" ") failed at " "trickles-userapi.c" "(%d)\n", 3117); };
                unlink((struct alloc_head*)relink);

                relink->sk = sk;
                if (!(({ struct tcp_opt *_tp = &((sk)->tp_pinfo.af_tcp); (((((char*)((relink)) - (char*)((_tp)->cminisock_api_config.cfg.ctl->minisock_base)) % sizeof(struct cminisock)) == 0) && (((void*)((struct cminisock *)((relink)))) >= ((void*)((_tp)->cminisock_api_config.cfg.ctl->minisock_base)) && ((void*)((struct cminisock *)((relink)) + 1)) <= ((_tp)->cminisock_api_config.cfg.ctl->minisock_limit))) && (((relink))->ctl == ALLOC_READY || ((relink))->ctl == ALLOC_HALFFREE); }))) { printk("KERNEL: assertion (" "IS_VALID_MSK(sk, relink)" ") failed at " "trickles-userapi.c" "(%d)\n", 3121); };
                switch(relink->ctl) {
                case ALLOC_READY:
                ...
                if(!((relink)->list == &(tp)->cminisock_api_config.msk_freelist || (relink)->list == &(tp)->cminisock_api_config.cfg.ctl->msk_eventlist)) {
        ...
        if(sk) {
                ...
                if(new_msk_list.len > 0) {
                        ...
                        while(new_msk_list.len > 0) {
                                ...
                                if (!(toFree->list == &tp->cminisock_api_config.msk_freelist)) { printk("KERNEL: assertion (" "toFree->list == &tp->cminisock_api_config.msk_freelist" ") failed at " "trickles-userapi.c" "(%d)\n", 3162); };
                        }
                }
...
int extract_MSKs_from_lbb(struct sock *sk, struct alloc_head_list *new_msk_list,
        ...
        while((char*)lbb_get_pos(lbb) < desc_end) {
                ...
                if(convertMSK_in(sk, ((void *)0), msk, lbb, &foo) != 0) {
        ...
        if (!(actual_end <= desc_end)) { printk("KERNEL: assertion (" "actual_end <= desc_end" ") failed at " "trickles-userapi.c" "(%d)\n", 3211); };
...
static inline int analyze_msk_list_helper(struct sock *sk, int print) {
        ...
        if((((sk)->tp_pinfo.af_tcp.trickles_opt & 0x1) && ((sk)->state == TCP_LISTEN))) {
                ...
                for(msk = ctl->minisock_base; (void*)(msk+1) < (void*)ctl->minisock_limit; msk++) {
                        ...
                        switch(msk->ctl) {
                                case ALLOC_FREE: ctlName = "ALLOC_FREE"; break;;
                                case ALLOC_READY: ctlName = "ALLOC_READY"; break;;
                                case ALLOC_PENDING: ctlName = "ALLOC_PENDING"; break;;
                                case ALLOC_PROCESSING: ctlName = "ALLOC_PROCESSING"; break;;
                                case ALLOC_HALFFREE: ctlName = "ALLOC_HALFFREE"; break;;
                        }
                        if(print)
