...
# 17 "/home/ashieh/current/include/net/trickles.h"
# 1 "/home/ashieh/current/include/linux/stddef.h" 1
# 18 "/home/ashieh/current/include/net/trickles.h" 2
...
# 6 "/home/ashieh/current/include/linux/types.h" 2
...
# 5 "/home/ashieh/current/include/linux/uio.h" 2
...
extern int put_cmsg (struct msghdr *, int level, int type, int len,
		     void *data);
# 19 "/home/ashieh/current/include/net/trickles.h" 2
...
# 21 "/home/ashieh/current/include/net/trickles.h" 2
...
# 13 "/home/ashieh/current/include/linux/wait.h"
...
# 14 "/home/ashieh/current/include/linux/wait.h" 2
...
# 9 "/home/ashieh/current/include/linux/poll.h" 2
...
# 10 "/home/ashieh/current/include/linux/poll.h" 2
...
# 25 "/home/ashieh/current/include/linux/net.h"
...
# 5 "/home/ashieh/current/include/linux/mm.h" 2
...
# 11 "/home/ashieh/current/include/linux/poll.h" 2
...
# 27 "/home/ashieh/current/include/linux/skbuff.h"
...
# 26 "/home/ashieh/current/include/net/trickles.h" 2
...
# 473 "/home/ashieh/current/include/net/cminisock.h"
struct ucontdesc;
...
# 520 "/home/ashieh/current/include/net/cminisock.h"
...
# 1 "/home/ashieh/current/include/net/trickles_minisock_functions.h" 1
		  ...
		  static void free_trickles_pmsk_finish (struct sock *sk,
							 struct pminisock
							 *msk);
# 359 "/home/ashieh/current/include/net/trickles_minisock_functions.h"
...
# 376 "/home/ashieh/current/include/net/trickles_minisock_functions.h"
...
# 394 "/home/ashieh/current/include/net/trickles_minisock_functions.h"
...
# 52 "state_cache.c"
		  ...
		  static inline struct pminisock_key pminisock_toKey (struct
								      cached_pminisock
								      *cmsk)
		  {
		  ...
		  static inline unsigned pminisock_hash (struct pminisock_key
							 *key)
		  {
		    return hash ((u1 *) & key->seq, sizeof (key->seq), 0);
		  }

		  static inline int pminisock_cmp (struct pminisock_key *k0,
						   struct cached_pminisock
						   *cmsk0,
						   struct pminisock_key *k1,
						   struct cached_pminisock
						   *cmsk1)
		  {

		    return k0->seq == k1->seq;
		  }
		  ...
		  static void pminisock_evict (void *context,
					       struct cached_pminisock *cmsk)
		  {
...
# 117 "state_cache.c"
		  struct ParsedPMinisock_cell
		  {
		    struct pminisock_key key;
		    struct cached_pminisock elem;
		  };
		  struct ParsedPMinisock_bucket
		  {
		    struct ParsedPMinisock_cell fastCell;
		  };
		  struct ParsedPMinisock_cache_root
		  {
		    struct ParsedPMinisock_bucket buckets[(128)];
		    int count;
		    int numBuckets;
		    int maxCount;
		    int hitCount;
		    ...
		    int lookupCount;
		    void *evictContext;
		    struct ParsedPMinisock_cell *cells;
		  } ParsedPMinisock_cache;
		  static inline int ParsedPMinisock_isFastCell (struct
								ParsedPMinisock_cell
								*cell)
		  {
		    struct ParsedPMinisock_cache_root *cache =
		      &ParsedPMinisock_cache;
		    return cell >=
		      (struct ParsedPMinisock_cell *) cache->buckets
		      && cell <
		      (struct ParsedPMinisock_cell *) (cache->buckets +
						       (128));
		  } static struct ParsedPMinisock_cell
		    *ParsedPMinisock_newCell (void);
		  static void ParsedPMinisock_deleteCell (struct
							  ParsedPMinisock_cell
							  *cell);
		  static int ParsedPMinisock_integrityCheck (void);
		  static void ParsedPMinisock_evictOne (struct
							ParsedPMinisock_cell
							**cell);
		  static int ParsedPMinisock_evict (int toEvict);
		  static void ParsedPMinisock_cache_init (void)
		  {
		    struct ParsedPMinisock_cache_root *cache =
		      &ParsedPMinisock_cache;
		    cache->count = 0;
		    cache->numBuckets = (128);
		    cache->maxCount = (128);
		    cache->hitCount = 0;
		    cache->total = 0;
		    cache->cellMem = 0;
		    ...
		    for (i = 0; i < cache->numBuckets; i++)
		      {
			struct ParsedPMinisock_bucket *bucket =
			  &cache->buckets[i];
			*(int *) &(bucket)->fastCell.elem = 0;
		      };
		  } static void ParsedPMinisock_cache_invalidate (void)
		  {
		    struct ParsedPMinisock_cache_root *cache =
		      &ParsedPMinisock_cache;
		    int i;
			...
			struct ParsedPMinisock_bucket *bucket =
			  &cache->buckets[i];
			if ((*(int *) &(bucket)->fastCell.elem != 0))
			  {
			    pminisock_evict (cache->evictContext,
					     &bucket->fastCell.elem);
			    ParsedPMinisock_deleteCell (&bucket->fastCell);
			  }
		  }} static void ParsedPMinisock_cache_destroy (void)
		  {
		    ParsedPMinisock_cache_invalidate ();
		  } static int ParsedPMinisock_getCount (void)
		  {;
		    return ParsedPMinisock_cache.count;
		  }
		  static int ParsedPMinisock_integrityCheck (void)
		  {
		    struct ParsedPMinisock_cache_root *cache =
			...
			if ((*(int *) &(bucket)->fastCell.elem != 0))
			  {
			    struct ParsedPMinisock_cell *cell =
			      &bucket->fastCell;
			    ((pminisock_hash (&cell->key) %
			      ParsedPMinisock_cache.numBuckets) == i) || (((
									      {
									      printk
									      ("pminisock hash index integrity"
									       "\n");
									      1;
									      }))
									  && (
			...
			if (!(lookupCount == cache->count))
			  {
			    printk ("(%s) failed at %s:%s():%d\n",
				    ...
				    "state_cache.c", __PRETTY_FUNCTION__,
				    120);
			    return 0;
			  }
		      }
		  ...
		  static void ParsedPMinisock_resetStats (void)
		  {
		    struct ParsedPMinisock_cache_root *cache =
		      &ParsedPMinisock_cache;
		    cache->hitCount = 0;
		    ...
		    *ParsedPMinisock_findInBucket (struct
						   ParsedPMinisock_bucket
						   *bucket,
						   struct pminisock_key *key)
		  {
		    struct ParsedPMinisock_cache_root *cache =
		    ...
		    if ((*(int *) &(bucket)->fastCell.elem != 0))
		      {
			struct ParsedPMinisock_cell *cell = &bucket->fastCell;
			if (pminisock_cmp
			    (key, ((void *) 0), &cell->key, &cell->elem))
			  {
			    return cell;
			  }
		      }
		    ...
		    *ParsedPMinisock_insert_helper (struct pminisock_key *key)
		  {
		    struct ParsedPMinisock_cache_root *cache =
		      &ParsedPMinisock_cache;
		    struct ParsedPMinisock_bucket *bucket =
		      &cache->buckets[pminisock_hash (key) %
				      cache->numBuckets];
		    struct ParsedPMinisock_cell *cell = ((void *) 0);
		    if (!(*(int *) &(bucket)->fastCell.elem != 0))
		      {
			cell = &bucket->fastCell;
		      ...
		      {
			cell = &bucket->fastCell;
			pminisock_evict (cache->evictContext, &cell->elem);
		  ...
		  static int ParsedPMinisock_insert (struct cached_pminisock
						     *elem)
		  {;
		    struct pminisock_key key = pminisock_toKey (elem);
		    struct ParsedPMinisock_cell *cell =
		      ParsedPMinisock_insert_helper (&key);
		    if (cell == ((void *) 0))
		      return 0;
		    cell->elem = *elem;;
		    return 1;
		  }
		  static int ParsedPMinisock_find_helper (struct pminisock_key
							  *key,
							  struct
							  ParsedPMinisock_cell
							  **hint)
		  {
		    ...
		    struct ParsedPMinisock_bucket *bucket =
		      &cache->buckets[pminisock_hash (key) %
				      cache->numBuckets];
		    struct ParsedPMinisock_cell *lookup;
		    lookup = ParsedPMinisock_findInBucket (bucket, key);
		    if (lookup != ((void *) 0))
		      {
			*hint = lookup;
		      };
		  ...
		  static int ParsedPMinisock_find (struct pminisock_key *key,
						   struct cached_pminisock
						   *result)
		  {
		    struct ParsedPMinisock_cell *cell = ((void *) 0);
		    int found = ParsedPMinisock_find_helper (key, &cell);
		    if (found)
		      {
			*result = cell->elem;
		      };
		  ...
		  static void ParsedPMinisock_freeCell (struct
							ParsedPMinisock_cell
							*cell);
		  static void ParsedPMinisock_deleteCell (struct
							  ParsedPMinisock_cell
							  *cell)
		  {
		    ParsedPMinisock_freeCell (cell);
		  } static int ParsedPMinisock_delete (struct pminisock_key
						       *key)
		  {
		    struct ParsedPMinisock_cell *cell = ((void *) 0);
		    int found = ParsedPMinisock_find_helper (key, &cell);
		  ...
		  {
		    struct ParsedPMinisock_bucket *bucket =
		      ((struct ParsedPMinisock_bucket *) (((char *) cell) -
							  ((char *)
							   &((struct
							      ParsedPMinisock_bucket
							      *) 0)->
...
# 126 "state_cache.c"
		  struct pminisock *pminisock_lookup (struct sock *sk,
						      __u32 seqno,
						      struct iphdr *iph,
						      struct tcphdr *th)
		  {
...
# 153 "state_cache.c"
			...
			if ((0))
			  {
			    printk
			      ("Pminisock lookup does not perform validation yet\n");
			  }
		  ...
		  int pminisock_insert (struct sock *sk,
					struct pminisock *pmsk)
		  {
		    ;
		    struct pminisock_key key;
		  ...
		  static inline CachedWireContinuationKey seqnoToKey (__u32
								      seqno)
		  {
		    return ntohl (seqno);
		  }

		  static inline CachedWireContinuationKey
		    CachedWireContinuation_toKey (struct
						  CachedWireContinuation *msk)
		  {
		    return seqnoToKey (msk->hdr.seq);
		  }

		  static inline unsigned
		    CachedWireContinuation_hash (CachedWireContinuationKey *
						 key)
		  {
		    return hash ((u1 *) key, sizeof (*key), 0);
		  }

		  static inline int CachedWireContinuation_cache_cmp (unsigned
								      *k0,
								      struct
								      CachedWireContinuation
								      *wcont0,
								      unsigned
								      *k1,
								      struct
		  ...
		  static inline void CachedWireContinuation_evict (void
								   *context,
								   struct
								   CachedWireContinuation
								   *cwc)
		  ...
		  static inline int nonce_cmp (NonceKey * k0,
					       struct NonceMap *map0,
					       NonceKey * k1,
					       struct NonceMap *map1)
		  {
		    return *k0 == *k1;
		  }

		  static inline void nonce_evict (void *context,
						  struct NonceMap *m)
		  {

		  }
# 269 "state_cache.c"
		  struct Continuation_cell
		  {
		    CachedWireContinuationKey key;
		    struct CachedWireContinuation elem;
		  };
		  ...
		  {
		    struct Continuation_bucket buckets[(128)];
		    int count;
		    int numBuckets;
		    int maxCount;
		    int hitCount;
		    ...
		    int lookupCount;
		    void *evictContext;
		    struct Continuation_cell *cells;
		  } Continuation_cache;
		  static inline int Continuation_isFastCell (struct
							     Continuation_cell
							     *cell)
		  {
		    struct Continuation_cache_root *cache =
		      &Continuation_cache;
		    return cell >= (struct Continuation_cell *) cache->buckets
		      && cell <
		      (struct Continuation_cell *) (cache->buckets + (128));
		  } static struct Continuation_cell
		    *Continuation_newCell (void);
		  static void Continuation_deleteCell (struct
						       Continuation_cell
						       *cell);
		  static int Continuation_integrityCheck (void);
		  static void Continuation_evictOne (struct Continuation_cell
						     **cell);
		  static int Continuation_evict (int toEvict);
		  static void Continuation_cache_init (void)
		  {
		    struct Continuation_cache_root *cache =
		      &Continuation_cache;
		    cache->count = 0;
		    ...
		    for (i = 0; i < cache->numBuckets; i++)
		      {
			struct Continuation_bucket *bucket =
			  &cache->buckets[i];
			*(int *) &(bucket)->fastCell.elem = 0;
		      };
		  } static void Continuation_cache_invalidate (void)
		  {
		    struct Continuation_cache_root *cache =
		      &Continuation_cache;
		    int i;
			...
			if ((*(int *) &(bucket)->fastCell.elem != 0))
			  {
			    CachedWireContinuation_evict (cache->evictContext,
							  &bucket->fastCell.
							  elem);
			    Continuation_deleteCell (&bucket->fastCell);
			  }
		  }} static void Continuation_cache_destroy (void)
		  {
		    Continuation_cache_invalidate ();
		  } static int Continuation_getCount (void)
		  {;
		    return Continuation_cache.count;
		  }
		  static int Continuation_integrityCheck (void)
		  {
		    struct Continuation_cache_root *cache =
			...
			if ((*(int *) &(bucket)->fastCell.elem != 0))
			  {
			    ...
			    ((CachedWireContinuation_hash (&cell->key) %
			      Continuation_cache.numBuckets) == i) || (((
									   {
									   printk
									   ("hash index integrity %X %X, %d %d"
									    "\n",
									    cell->
									    key,
		  ...
		  static void Continuation_resetStats (void)
		  {
		    struct Continuation_cache_root *cache =
		      &Continuation_cache;
		    cache->hitCount = 0;
		    ...
		    *Continuation_findInBucket (struct Continuation_bucket
						*bucket,
						CachedWireContinuationKey *
						key)
		  {
		    struct Continuation_cache_root *cache =
		      &Continuation_cache;
		    ...
		    if ((*(int *) &(bucket)->fastCell.elem != 0))
		      {
			struct Continuation_cell *cell = &bucket->fastCell;
			if (CachedWireContinuation_cache_cmp
			    (key, ((void *) 0), &cell->key, &cell->elem))
			  {
			    return cell;
			  }
		    ...
		    *Continuation_insert_helper (CachedWireContinuationKey *
						 key)
		  {
		    struct Continuation_cache_root *cache =
		      &Continuation_cache;
		    struct Continuation_bucket *bucket =
		      &cache->buckets[CachedWireContinuation_hash (key) %
				      cache->numBuckets];
		    struct Continuation_cell *cell = ((void *) 0);
		    if (!(*(int *) &(bucket)->fastCell.elem != 0))
		      {
			cell = &bucket->fastCell;
		      }
		  ...
		  static int Continuation_insert (struct
						  CachedWireContinuation
						  *elem)
		  {;
		    CachedWireContinuationKey key =
		      CachedWireContinuation_toKey (elem);
		    struct Continuation_cell *cell =
		      Continuation_insert_helper (&key);
		    if (cell == ((void *) 0))
		      return 0;
		    cell->elem = *elem;;
		    return 1;
		  ...
		  static int
		    Continuation_find_helper (CachedWireContinuationKey * key,
					      struct Continuation_cell **hint)
		  {
		    struct Continuation_cache_root *cache =
		      &Continuation_cache;
		    struct Continuation_bucket *bucket =
		      &cache->buckets[CachedWireContinuation_hash (key) %
				      cache->numBuckets];
		    struct Continuation_cell *lookup;
		    lookup = Continuation_findInBucket (bucket, key);
		    if (lookup != ((void *) 0))
		      {
			*hint = lookup;
		      };
		  ...
		  static int Continuation_find (CachedWireContinuationKey *
						key,
						struct CachedWireContinuation
						*result)
		  {
		    struct Continuation_cell *cell = ((void *) 0);
		    int found = Continuation_find_helper (key, &cell);
		    if (found)
		      {
			*result = cell->elem;
		      };
		  ...
		  static void Continuation_freeCell (struct Continuation_cell
						     *cell);
		  static void Continuation_deleteCell (struct
						       Continuation_cell
						       *cell)
		  {
		    Continuation_freeCell (cell);
		  } static int Continuation_delete (CachedWireContinuationKey
						    * key)
		  {
		    struct Continuation_cell *cell = ((void *) 0);
		    int found = Continuation_find_helper (key, &cell);
		  ...
		  {
		    struct Continuation_bucket *bucket =
		      ((struct Continuation_bucket *) (((char *) cell) -
						       ((char *)
							&((struct
							   Continuation_bucket
							   *) 0)->fastCell)));
...
# 289 "state_cache.c"
		  ...
		  static inline int NonceKey_isFastCell (struct NonceKey_cell
							 *cell)
		  {
		    struct NonceKey_cache_root *cache = &NonceKey_cache;
		    return cell >= (struct NonceKey_cell *) cache->buckets
		      && cell <
		      (struct NonceKey_cell *) (cache->buckets + (32));
		  } static struct NonceKey_cell *NonceKey_newCell (void);
		  static void NonceKey_deleteCell (struct NonceKey_cell
						   *cell);
		  static int NonceKey_integrityCheck (void);
		  static void NonceKey_evictOne (struct NonceKey_cell **cell);
		  static int NonceKey_evict (int toEvict);
		  static void NonceKey_cache_init (void)
		  {
		    struct NonceKey_cache_root *cache = &NonceKey_cache;
		    cache->count = 0;
		    cache->numBuckets = (32);
			...
			*(int *) &(bucket)->fastCell.elem = 0;
			...
			NonceKey_integrityCheck () || (
			  ...
			  } static void NonceKey_cache_invalidate (void)
			  {
			    struct NonceKey_cache_root *cache =
			      &NonceKey_cache;
			    int i;
			    for (i = 0; i < cache->numBuckets; i++)
			      {
				struct NonceKey_bucket *bucket =
				  &cache->buckets[i];
				if ((*(int *) &(bucket)->fastCell.elem != 0))
				  {
				    nonce_evict (cache->evictContext,
						 &bucket->fastCell.elem);
				    NonceKey_deleteCell (&bucket->fastCell);
				  }
			  }} static void NonceKey_cache_destroy (void)
			  {
			    NonceKey_cache_invalidate ();
			  } static int NonceKey_getCount (void)
			  {
			    if ((0) >= (2))
			      {
				NonceKey_integrityCheck () || (
...
|| (((
     ...
     1;
	      ...
	      1;
					...
					struct NonceKey_bucket *bucket =
					  &cache->buckets[i];
					if ((*(int *) &(bucket)->fastCell.
					     elem != 0))
					  {
					    struct NonceKey_cell *cell =
					      &bucket->fastCell;
					    ((nonce_hash (&cell->key) %
					      NonceKey_cache.numBuckets) == i)
					      || (((
						       {
						       printk
				  ...
				  static void NonceKey_resetStats (void)
				  {
				    struct NonceKey_cache_root *cache =
				      &NonceKey_cache;
				    cache->hitCount = 0;
				    cache->total = 0;
				  } static struct NonceKey_cell
				    *NonceKey_findInBucket (struct
							    NonceKey_bucket
							    *bucket,
							    __u64 * key)
				  {
				    ...
				    *NonceKey_insert_helper (__u64 * key)
				  {
				    struct NonceKey_cache_root *cache =
				      &NonceKey_cache;
				    struct NonceKey_bucket *bucket =
				      &cache->buckets[nonce_hash (key) %
						      cache->numBuckets];
				    struct NonceKey_cell *cell = ((void *) 0);
				    if (!
					(*(int *) &(bucket)->fastCell.elem !=
					 0))
				      {
					cell = &bucket->fastCell;
				      }
				  ...
				  static int NonceKey_insert (struct NonceMap
							      *elem)
				  {
				    if ((0) >= (2))
				      {
									...
									*cell
									  ...
									  (2))
									{
									  NonceKey_integrityCheck
										...
										*bucket
										=
										((struct NonceKey_bucket *) (((char *) cell) - ((char *) &((struct NonceKey_bucket *) 0)->fastCell)));
									      *(int *) &(bucket)->fastCell.elem = 0;
									    };
# 298 "state_cache.c"
									    void
...
# 329 "state_cache.c"
										...
										*phdr
										...
										dest;
									      ...
									      (__builtin_constant_p ((((__u8 *) (((WireContinuation *) 0) + 1) - (__u8 *) ((WireContinuation *) 0)->hmac_start))) ? __constant_memcpy ((c->copy_start), (dcont->hmac_start), ((((__u8 *) (((WireContinuation *) 0) + 1) - (__u8 *) ((WireContinuation *) 0)->hmac_start)))) : __memcpy ((c->copy_start), (dcont->hmac_start), ((((__u8 *) (((WireContinuation *) 0) + 1) - (__u8 *) ((WireContinuation *) 0)->hmac_start)))));
									    }

									    static
									      inline
									      int
									      WireContinuation_checkAndCopyOut
									      (struct
									       WireContinuation
									       *w,
									       struct
										...
										*phdr
										...
										&c->
										hdr;
									      if (phdr->seq == w->seq && phdr->type == w->continuationType && phdr->first == w->firstChild && phdr->serverAddr == iph->daddr && phdr->serverPort == th->dest && phdr->clientAddr == iph->saddr && phdr->clientPort == th->source && w->tokenCounterBase == c->tokenCounterBase)
										{
										  (__builtin_constant_p ((((__u8 *) (((WireContinuation *) 0) + 1) - (__u8 *) ((WireContinuation *) 0)->hmac_start))) ? __constant_memcpy ((w->hmac_start), (c->copy_start), ((((__u8 *) (((WireContinuation *) 0) + 1) - (__u8 *) ((WireContinuation *) 0)->hmac_start)))) : __memcpy ((w->hmac_start), (c->copy_start), ((((__u8 *) (((WireContinuation *) 0) + 1) - (__u8 *) ((WireContinuation *) 0)->hmac_start)))));
										  return
										    1;
										}
									      else
										...
										(w->
										 ...
										 c->
										 copy_start,
										 (((__u8 *) (((WireContinuation *) 0) + 1) - (__u8 *) ((WireContinuation *) 0)->hmac_start)));
									    }

									    int
									      WireContinuation_lookup
									      (struct
									       WireContinuation
									       *cont,
									       struct
...
# 382 "state_cache.c"
										  if (!WireContinuation_checkAndCopyOut (cont, &cell->elem, iph, th))
											...
											{
											  printk
											    ("Sequence number and flow ID did not match\n");
											  return
											    0;
											}
										    }
