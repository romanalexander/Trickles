	      ...
	      if (0)
		{
							   ...
							   *ucont)
		  {
		    ...
		    printk ("valid=[%d-%d] [%d-%d]\n", ucont->validStart,
			    ucont->validEnd, htonl (ucont->validStart),
			    htonl (ucont->validEnd));
		    printk ("cvalid=[%d-%d] [%d-%d]\n",
			    ucont->clientValidStart, ucont->clientValidEnd,
			    htonl (ucont->clientValidStart),
			    htonl (ucont->clientValidEnd));
		  }
		  ...
		  static inline struct RequestChunk
		    *pushhint_buildChunkHeader (struct RequestChunk *chunk,
						int start, int end)
		  {
						   ...
						   *ctx, int numBytes)
		  {
			...
			if (!(numBytes <= ctx->packets[ctx->packetNum].len))
			  {
			    printk ("KERNEL: assertion ("
				    "numBytes <= ctx->packets[ctx->packetNum].len"
				    ") failed at "
				    "/home/ashieh/current/include/net/trickles_packet_helpers.h"
				    "(%d)\n", 860);
			  };
			ctx->outputPos +=
			  ctx->packets[ctx->packetNum].len - ctx->packetPos;
			ctx->packetPos = 0;
			ctx->packetNum++;
		      }
		    temp = ctx->outputPos;
		    ...
		    if (!(ctx->packetPos <= ctx->packets[ctx->packetNum].len))
		      {
			printk ("KERNEL: assertion ("
				"ctx->packetPos <= ctx->packets[ctx->packetNum].len"
				") failed at "
				"/home/ashieh/current/include/net/trickles_packet_helpers.h"
				"(%d)\n", 871);
		      };
					      ...
					      struct pminisock *msk)
		  {
			...
			if (!!
			    (msk >=
			     (struct pminisock *) tp->cminisock_api_config.
			     cfg.ctl->pminisock_base
			     && (msk + 1) <=
			     (struct pminisock *) tp->cminisock_api_config.
			     cfg.ctl->pminisock_limit))
			  {
			    if (!
				(((sk)->tp_pinfo.af_tcp.trickles_opt & 0x1)
				 && !((sk)->tp_pinfo.af_tcp.
		  ...
		  static inline int analyze_msk_list_helper (struct sock *sk,
							     int print);
		  ...
		  inline void dump_socket_stats (struct sock *sk)
		  {
		    struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
		    printk ("   tp->rcv_nxt = %d\n", tp->rcv_nxt);
		    ...
		    (
		      {
		      struct tcp_opt * tp = &sk->tp_pinfo.af_tcp;
		      printk ("A = %d, D = %d, RTO = %d", tp->t.A, tp->t.D,
			      tp->t.RTO);
		      });
		  ...
		  static inline void dump_datachunks (char *ptr, int len)
		  {
		    int chunknum = 0;
		  ...
		  static inline void
		    fill_page_desc (struct sk_buff *skb, int i,
				    struct page *page, int off, int size)
		  {
		    skb_frag_t *frag =
		      &((struct skb_shared_info *) ((skb)->end))->frags[i];
		    frag->page = page;
		    ...
		    skb_add_data (struct sk_buff *skb, char *from, int copy)
		  {
		    ...
		    csum =
		      csum_and_copy_from_user (from, dest, copy, 0, &err);
		    if (!err)
		      {
		  ...
		  static inline void init_sock (struct cminisock *msk,
						struct sock *sk)
		  {
		    msk->sk = sk;
						...
						long) (((((mem_map +
							   ((unsigned
							     long) (((pte).
								     ...
								     12))))) -
							 page_zone ((mem_map +
								     ((unsigned long) (((pte).pte_low >> 12)))))->zone_mem_map) << 12) + page_zone ((mem_map + ((unsigned long) (((pte).pte_low >> 12)))))->zone_start_paddr) + ((unsigned long) (0xC0000000))));
		  ...
		  inline void queue_upcall_msk_prealloc (struct sock *sk,
							 enum
							 cminisock_event_tag
							 tag,
							 struct cminisock
							 *msk)
		  {
		    ...
		    switch (tag)
		      ...
		      case FIN:
		      case ACK:
			insert_tail_mb (&tp->cminisock_api_config.cfg.ctl->
					msk_eventlist,
					(struct alloc_head *) msk);;
			break;
		      default:
		  ...
		  inline void queue_upcall_msk (enum cminisock_event_tag tag,
						struct cminisock *msk)
		  {
		    msk->tag = tag;
		    switch (tag)
		  ...
		  inline void queue_upcall_pmsk_prealloc (struct sock *sk,
							  enum
							  cminisock_event_tag
							  tag,
							  struct pminisock
							  *msk)
		  {
		    ...
		    switch (tag)
		      ...
		      case FIN:
		      case ACK:
			dlist_insert_tail_mb (&tp->cminisock_api_config.cfg.
					      ctl->pmsk_eventlist,
					      (struct list_link *) msk);;
			break;
		      default:
			printk ("Unsupported tag for preallocation\n");
		      }
		  }
		  inline void queue_upcall_pmsk (enum cminisock_event_tag tag,
						 struct pminisock *msk)
		  {
		    msk->tag = tag;
		    switch (tag)
		  ...
		  static inline void new_event (struct sock *sk)
		  {
		    __wake_up ((sk->sleep), 1, 1);
		    if (!sk->dead)
		      {
		    ...
		    atomic_inc (&sk->tp_pinfo.af_tcp.cminisock_api_config.cfg.
				ctl->update_since_poll);
		  }
		  ...
		  static void tcp_prequeue_process (struct sock *sk)
		  {
		    ...
		    net_statistics[0 * 2 + 1].TCPPrequeued +=
		    ...
		    while (0);
		    while ((skb =
			    __skb_dequeue (&tp->ucopy.prequeue)) !=
			   ((void *) 0))
			...
			__asm__ __volatile__ ("":::"memory");
			if (!--*ptr)
			  __asm__ __volatile__ ("cmpl $0, -8(%0);" "jnz 2f;"
						"1:;" ".subsection 1\n\t" ""
						".ifndef " ".text.lock."
						"tmalloc" "\n\t" ".text.lock."
						"tmalloc" ":\n\t" ".endif\n\t"
		  ...
		  static inline int tiov_handler (struct sock *origSK,
						  struct cminisock *msk,
						  struct tiovec *tiov,
						  int tiovlen);
		  static inline int trickles_do_sendmsg_tiov (struct sock *sk,
							      struct cminisock
							      *msk, void *vec,
							      int veclen)
		  {
		    ...
		    int rval = 0;
		    struct sock dummysk;
		    struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp), *dummy_tp =
		      &(dummysk.tp_pinfo.af_tcp);
		    struct sk_buff *skb;
		    ...
		    if (msk->tag == SYN || msk->tag == ACK || msk->tag == FIN)
		      {
			tiov_handler (sk, msk, vec, veclen);
		      }
		    else
		      {
			...
			switch (msk->tag)
			  ...
			  default:
			    ...
			    if (!(0))
			      {
				printk ("KERNEL: assertion (" "0"
					") failed at " "trickles-userapi.c"
					"(%d)\n", 342);
			      };
			    goto out;
			  }
			...
			for (i = 0; i < msk->num_packets; i++)
			  {
			    struct sk_buff *skb1 = skb_clone (skb, (0x20));
			    skb1->csum = 0;
			    skb1->ip_summed = 1;
			...
			&& ((msk)->cacheRecycleIndex >= 0))
		      {
			pmsk_hold (pmsk);
			pminisock_cache_child (sk, msk, pmsk,
					       pmsk->cacheRecycleIndex, 0);
			free_trickles_pmsk_finish (sk, pmsk);
		  ...
		  static inline int fiov_handler (struct sock *origSK,
						  struct cminisock *msk,
						  struct fiovec *fiov,
						  int fiovlen);
		  static inline int trickles_do_sendmsg_fiov (struct sock *sk,
							      struct cminisock
							      *msk, void *vec,
							      int veclen)
		  {
		    ...
		    if (msk->tag == SYN || msk->tag == ACK || msk->tag == FIN)
		      {
			fiov_handler (sk, msk, vec, veclen);
		      }
		    else
		      {
			struct tcp_skb_cb *tcb;
		  ...
		  static inline
		    void free_remaining (struct sk_buff **skbs, int offset,
					 int total)
		  {
		  ...
		  static
		    void finishTransmitHelper (struct cminisock *msk,
					       struct sk_buff *skbs[],
					       int numDataPackets,
					       struct ucontdesc *udesc,
						  ...
						  int tiovlen)
		  {
		    ...
		    int i, numDataPackets = 0, totallen = 0, tiov_pos =
		      0, tiov_offs = 0;
			...
			if (!(msk->num_packets >= 0))
			  {
			    printk ("KERNEL: assertion ("
				    "msk->num_packets >= 0" ") failed at "
				    "trickles-userapi.c" "(%d)\n", 517);
			  };
			skbs = skb_body;
		      }
		    int maxLen = 0;
			...
			if ((0))
			  {
			    printk
			      ("%lud:mark(%d) Short by %d (%d - %d) count=(%d,%d)\n",
			...
			while (tiov_pos < tiovlen)
			  {
			    int amt = ({ typeof (pkt_remain) Z =
				       (typeof (pkt_remain)) (tiov[tiov_pos].
							      iov_len -
							      tiov_offs);
				       (
					 {
					 const typeof (pkt_remain) _x =
					 (pkt_remain);
					 const typeof (Z) _y = (Z);
					 (void) (&_x == &_y);
					 _x < _y ? _x : _y;
			    ...
			    if (err)
			      {
				printk
				  ("error while csum/copy, base = %p amt = %d, msk state = %d, (seq,base,delta) = (%d,%d,%d)\n",
				   src, amt, msk->state, msk->seq,
				   msk->TCPBase, msk->seq - msk->TCPBase);
			...
			if (!(pkt_remain == 0))
			  {
			    printk ("KERNEL: assertion (" "pkt_remain == 0"
				    ") failed at " "trickles-userapi.c"
				    "(%d)\n", 600);
			  };
			...
			if (userapi_pkt_spew)
			  {
			    printk ("sending pkts[%d] %u-%u\n", i,
				    pkts[i].seq, pkts[i].seq + pkts[i].len);
			  }


			if (totallen == 0 &&
			    tiov[tiovlen - 1].iov_base == (void *) -1
			    && tiov[tiovlen - 1].iov_len == 0)
			  {
			    printk ("Fin, iovlen = %d\n", tiovlen);
			    msk->tag = FIN;
			  }
		    ...
		    finishTransmitHelper (msk, skbs, numDataPackets,
					  ((void *) 0), 0, 0);
			...
			if ((0))
			  {
			    int i;
			    printk
			      ("too much data for downcall: %d left, trace = %d\n",
			       totallen, msk->executionTrace);
			    for (i = 0; i < tiovlen; i++)
			      {
				printk ("tiov[%d] = {base = %p, len = %d}\n",
					i, tiov[i].iov_base, tiov[i].iov_len);
			      }
			  }
		  ...
		  void trickles_close (struct sock *sk, long timeout)
		  {
		    ...
		    while ((skb =
			    __skb_dequeue (&sk->receive_queue)) !=
			   ((void *) 0))
		      {
			u32 len =
			...
			while (0);
			if ((sk)->backlog.tail != ((void *) 0))
			  __release_sock (sk);
			(sk)->lock.users = 0;
			if (waitqueue_active (&((sk)->lock.wq)))
			  __wake_up ((&((sk)->lock.wq)), 2 | 1, 1);
			do
			  {
			    do
			      {
		  ...
		  void trickles_init_sock_impl (struct sock *sk, int val)
		  {
		    ...
		    while (0);
		    (void) (&((sk)->lock.slock));
		    if (!(tp->trickles_opt & 0x1) && val & 0x1)
		      {
			if (val & 0x8)
			  {
			    tp->t.testseq = 1;
					...
					int size)
		  {
		    ...
		    if (sizeof (*cmd) != msg->msg_namelen
			|| (int) cmd->magic != (0xbaadd33d))
		      {
			return -22;
		      }
		    ...
		    if (!(
			   ...
			   (((((char *) ((msk)) -
			       (char *) ((_tp)->cminisock_api_config.cfg.ctl->
					 minisock_base)) %
			      sizeof (struct cminisock)) == 0)
			    && (((void *) ((struct cminisock *) ((msk)))) >=
				((void *) ((_tp)->cminisock_api_config.cfg.
					   ctl->minisock_base))
				&&
				((void *) ((struct cminisock *) ((msk)) +
					   1)) <=
				((_tp)->cminisock_api_config.cfg.ctl->
				 minisock_limit)))
			   && (((msk))->ctl == ALLOC_READY
			       || ((msk))->ctl == ALLOC_HALFFREE);}))
		      {
			printk ("bad minisocket %d %d\n",
				!(((((char *) (msk) -
				     (char *) ((tp)->cminisock_api_config.cfg.
					       ctl->minisock_base)) %
				    sizeof (struct cminisock)) == 0)
				  && (((void *) ((struct cminisock *) (msk)))
				      >=
				      ((void *) ((tp)->cminisock_api_config.
						 cfg.ctl->minisock_base))
				      &&
				      ((void *) ((struct cminisock *) (msk) +
						 1)) <=
		    ...
		    rval =
		      trickles_do_sendmsg_tiov (sk, api_msk, tiov, tiovlen);
		  ...
		  static int trickles_poll (struct sock *sk)
		  {
		    ...
		    struct socket sock;
		    sock.sk = sk;
		    if ((trickles_sock_poll_impl
			 (((void *) 0), &sock, ((void *) 0)) == 0))
		      {
					       ...
					       poll_table * wait)
		  {
		    ...
		    int mask = 0;
		    if (((tp)->cminisock_api_config.cfg.mmap_base !=
			 ((void *) 0)))
		      {
			int poll_value =
			  ((&tp->cminisock_api_config.cfg.ctl->
			    update_since_poll)->counter);
			if ((!(tp->trickles_opt & 0x80)
			     && !dlist_empty (&tp->cminisock_api_config.cfg.
					      ctl->pmsk_eventlist))
			    || ((tp->trickles_opt & 0x80) && poll_value > 0
				&& !dlist_empty (&tp->cminisock_api_config.
						 cfg.ctl->pmsk_eventlist)))
			  {
			    mask = 0x0001;
			    gUpdateSincePollTotal += poll_value;
			    gUpdateSincePollCount++;
			    atomic_sub (poll_value,
					&tp->cminisock_api_config.cfg.ctl->
					update_since_poll);
			  }
		      }
		  ...
		  static struct page *trickles_vma_nopage (struct
							   vm_area_struct
							   *area,
							   unsigned long
							   address,
							   int unused)
		  {
		    unsigned long offset;
		    struct page *page;
		    struct trickles_kconfig *api_config =
		      (struct trickles_kconfig *) area->vm_private_data;
		    if (api_config == ((void *) 0))
		      return ((void *) 0);
		    ...
		    page =
		      (mem_map +
		       (((unsigned
			  long) (virt_to_kseg ((char *) base + offset)) -
			 ((unsigned long) (0xC0000000))) >> 12));
		    if (page)
						  ...
						  int direction)
		  {
			...
			error =
			  (__builtin_constant_p (sizeof (new_config)) ?
			   __constant_copy_from_user ((&new_config),
						      (optdata),
						      (sizeof (new_config))) :
			   __generic_copy_from_user ((&new_config), (optdata),
						     (sizeof (new_config))));
			if (error)
			  {
			...
			if ((mmap_len =
			     new_config.mmap_len) > ((1 << 16) * (1UL << 12)))
			  {
			...
			new_config.ctl = ctl;
			if (!
			    (sizeof (*ctl) + sizeof (struct tq_struct) <
			     (1UL << 12)))
			  {
			    printk ("KERNEL: assertion ("
				    "sizeof(*ctl) + sizeof(struct work_struct) < PAGE_SIZE"
				    ") failed at " "trickles-userapi.c"
				    "(%d)\n", 991);
			  };
			...
			ctl->ro_offs =
			  (char *) ctl->ro_base -
			  (char *) new_config.mmap_base;
			ctl->rw_base = (__u8 *) ctl->ro_base + ctl->ro_len;
			ctl->rw_len =
			  (((mmap_len -
			     ((char *) ctl->rw_base -
			      (char *) new_config.mmap_base)) + (1UL << 12) -
			    1) & (~((1UL << 12) - 1))) - (1UL << 12);
			ctl->rw_offs =
			  (char *) ctl->rw_base -
			  (char *) new_config.mmap_base;
			...
			do
			  {
			    ...
			    while (0);
			    ((struct tq_struct *) (ctl + 1))->sync = 0;
			    do
			      {
				(((struct tq_struct *) (ctl + 1)))->routine =
				  (vfree_helper);
				(((struct tq_struct *) (ctl + 1)))->data =
				  (new_config.mmap_base);
			      }
			    while (0);
			  }
			...
			if (!(ctl->rw_len > 0))
			  {
			    printk ("KERNEL: assertion (" "ctl->rw_len > 0"
				    ") failed at " "trickles-userapi.c"
				    "(%d)\n", 1000);
			  };
			if (!(ctl->ro_len > 0))
			  {
			    printk ("KERNEL: assertion (" "ctl->ro_len > 0"
				    ") failed at " "trickles-userapi.c"
				    "(%d)\n", 1001);
			  };
			if (!
			    ((char *) ctl->ro_base <
			     (char *) new_config.mmap_base + mmap_len))
			  {
			    printk ("KERNEL: assertion ("
				    "(char*)ctl->ro_base < (char*)new_config.mmap_base + mmap_len"
				    ") failed at " "trickles-userapi.c"
				    "(%d)\n", 1002);
			  };
			...
			msk =
			  (struct cminisock *) ((char *) ctl->ro_base +
						(1UL << 12));
			ctl->minisock_base = msk;
			ctl->minisock_offs =
			  (char *) ctl->minisock_base -
			  (char *) new_config.mmap_base;

			minisock_len = ((ctl->ro_len) / 4);
			ctl->minisock_limit =
			  (char *) ctl->minisock_base + minisock_len;
			if (!(minisock_len > 0))
			  {
			    printk ("KERNEL: assertion (" "minisock_len > 0"
				    ") failed at " "trickles-userapi.c"
				    "(%d)\n", 1019);
			  };
			while ((char *) (msk + 1) <=
			       (char *) ctl->minisock_limit)
			  {
			    (__builtin_constant_p (0x3a)
			     ? (__builtin_constant_p ((sizeof (*msk))) ?
				__constant_c_and_count_memset (((msk)),
							       ((0x01010101UL
								 *
								 (unsigned
								  char)
								 (0x3a))),
							       ((sizeof
								 (*msk)))) :
				__constant_c_memset (((msk)),
						     ((0x01010101UL *
						       (unsigned
							char) (0x3a))),
						     ((sizeof (*msk)))))
			     : (__builtin_constant_p ((sizeof (*msk))) ?
				__memset_generic ((((msk))), (((0x3a))),
						  (((sizeof (*msk))))) :
				__memset_generic (((msk)), ((0x3a)),
						  ((sizeof (*msk))))));
			    msk->ctl = ALLOC_FREE;
			...
			ctl->pminisock_base =
			  ((typeof (ctl->minisock_limit)) (char *)
			   (ctl->minisock_limit) + (1 -
						    ((int) ctl->
						     minisock_limit) % 1) %
			   1);
			...
			ctl->pminisock_limit =
			  (char *) ctl->pminisock_base + pminisock_len;
			while ((char *) (pmsk + 1) <=
			       (char *) ctl->pminisock_limit)
			  {
			    (__builtin_constant_p (0x3a)
			     ? (__builtin_constant_p ((sizeof (*pmsk))) ?
				__constant_c_and_count_memset (((pmsk)),
							       ((0x01010101UL
								 *
								 (unsigned
								  char)
								 (0x3a))),
							       ((sizeof
								 (*pmsk)))) :
				__constant_c_memset (((pmsk)),
						     ((0x01010101UL *
						       (unsigned
							char) (0x3a))),
						     ((sizeof (*pmsk)))))
			     : (__builtin_constant_p ((sizeof (*pmsk))) ?
				__memset_generic ((((pmsk))), (((0x3a))),
						  (((sizeof (*pmsk))))) :
				__memset_generic (((pmsk)), ((0x3a)),
						  ((sizeof (*pmsk))))));
			...
			ctl->heap_base =
			  ((typeof (ctl->pminisock_limit)) (char *)
			   (ctl->pminisock_limit) + (1 -
						     ((int) ctl->
						      pminisock_limit) % 1) %
			   1);
			...
			error =
			  (__builtin_constant_p (sizeof (new_config)) ?
			   __constant_copy_to_user ((optdata), (&new_config),
						    (sizeof (new_config))) :
			   __generic_copy_to_user ((optdata), (&new_config),
						   (sizeof (new_config))));
			if (error)
			  {
			...
			if (generateHMAC)
			  {
			    ...
			    __u8 hmac_key[(16) + 1] =
			      "\00\01\02\03\04\05\06\07\010\011\012\013\014\015\016\017\020\021\022\023\024";



			    tp->t.hmacCTX =
			      kmalloc (sizeof (*tp->t.hmacCTX),
				       (0x20 | 0x10 | 0x40 | 0x80 | 0x100));
			...
			if ((
			      ...
			      switch (sizeof (*((int *)optlen)))
			      {
case 1:
__asm__ __volatile__ ("call __get_user_" "1": "=a" (__ret_gu), "=d" (__val_gu): "0" ((int *) optlen)); break; case 2:
__asm__ __volatile__ ("call __get_user_" "2": "=a" (__ret_gu), "=d" (__val_gu): "0" ((int *) optlen)); break; case 4:
__asm__ __volatile__ ("call __get_user_" "4": "=a" (__ret_gu), "=d" (__val_gu): "0" ((int *) optlen)); break; default:
			__asm__ __volatile__ ("call __get_user_" "X": "=a" (__ret_gu), "=d" (__val_gu):"0" ((int *) optlen));
			      break;}
			      (len) =
			      (__typeof__ (*((int *) optlen))) __val_gu;
			      __ret_gu;}))
			  {
			    printk
			      ("pipe parameter readout: get optlen fault\n");
			    return -14;
			  }
			error =
			  (__builtin_constant_p
			   (sizeof (tp->cminisock_api_config.cfg)) ?
			   __constant_copy_to_user ((optdata),
						    (&tp->
						     cminisock_api_config.
						     cfg),
						    (sizeof
						     (tp->
						      cminisock_api_config.
						      cfg))) :
			   __generic_copy_to_user ((optdata),
						   (&tp->cminisock_api_config.
						    cfg),
						   (sizeof
						    (tp->cminisock_api_config.
			...
			if (error)
			  {
			    printk
			      ("pipe parameter readout: copy out fault\n");
			    return -22;
			  }
			if ((
			      {
			      long __pu_err = -14;
			      __typeof__ (*(((int *) optlen))) * __pu_addr =
			      (((int *) optlen)); if (((
							   {
			unsigned long flag, sum; asm ("addl %3,%1 ; sbbl %0,%0; cmpl %1,%4; sbbl $0,%0": "=&r" (flag), "=r" (sum):"1" (__pu_addr), "g" ((int) (sizeof (*((int *)optlen)))), "g" (get_current ()->addr_limit.seg)); flag;}) == 0))
			      do
			      {
			      __pu_err = 0;
			      switch ((sizeof (*((int *)optlen))))
...
__asm__ __volatile__ ("1:	movl %%eax,0(%2)\n" "2:	movl %%edx,4(%2)\n" "3:\n" ".section .fixup,\"ax\"\n" "4:	movl %3,%0\n" "	jmp 3b\n" ".previous\n" ".section __ex_table,\"a\"\n" "	.align 4\n" "	.long 1b,4b\n" "	.long 2b,4b\n" ".previous": "=r" (__pu_err): "A" (((__typeof__ (*((int *) optlen))) (len))), "r" (__pu_addr), "i" (-14), "0" (__pu_err)); break; default:
			  ...
			  {
			    printk
			      ("pipe parameter readout: set optlen fault\n");
			    return -14;
			  }
			...
			out_dealloc:
			  ...
			  int trickles_getsockopt_impl (struct sock *sk,
							int level,
							int optname,
							char *optval,
							int *optlen)
			  {
			    ...
			    if ((
				  ...
				  {
case 1:
__asm__ __volatile__ ("call __get_user_" "1": "=a" (__ret_gu), "=d" (__val_gu): "0" (optlen)); break; case 2:
__asm__ __volatile__ ("call __get_user_" "2": "=a" (__ret_gu), "=d" (__val_gu): "0" (optlen)); break; case 4:
__asm__ __volatile__ ("call __get_user_" "4": "=a" (__ret_gu), "=d" (__val_gu): "0" (optlen)); break; default:
			    __asm__ __volatile__ ("call __get_user_" "X": "=a" (__ret_gu), "=d" (__val_gu):"0" (optlen));
				  break;}
				  (len) = (__typeof__ (*(optlen))) __val_gu;
				  __ret_gu;}
				))
			      {
				return -14;
			    ...
			    if ((
				  {
				  long __pu_err = -14;
				  __typeof__ (*((optlen))) * __pu_addr =
				  ((optlen)); if (((
						       {
			    unsigned long flag, sum; asm ("addl %3,%1 ; sbbl %0,%0; cmpl %1,%4; sbbl $0,%0": "=&r" (flag), "=r" (sum):"1" (__pu_addr), "g" ((int) (sizeof (*(optlen)))), "g" (get_current ()->addr_limit.seg)); flag;}
						   ) == 0))
				  do
				  {
				  __pu_err = 0; switch ((sizeof (*(optlen))))
				  {
case 1:
...
__asm__ __volatile__ ("1:	movl %%eax,0(%2)\n" "2:	movl %%edx,4(%2)\n" "3:\n" ".section .fixup,\"ax\"\n" "4:	movl %3,%0\n" "	jmp 3b\n" ".previous\n" ".section __ex_table,\"a\"\n" "	.align 4\n" "	.long 1b,4b\n" "	.long 2b,4b\n" ".previous": "=r" (__pu_err): "A" (((__typeof__ (*(optlen))) (outputLen))), "r" (__pu_addr), "i" (-14), "0" (__pu_err)); break; default:
				...
				return -14;
			      ...
			      if ((
				    {
				    long __pu_err = -14;
				    __typeof__ (*((optval))) * __pu_addr =
				    ((optval)); if (((
							 {
			      unsigned long flag, sum; asm ("addl %3,%1 ; sbbl %0,%0; cmpl %1,%4; sbbl $0,%0": "=&r" (flag), "=r" (sum):"1" (__pu_addr), "g" ((int) (sizeof (*(optval)))), "g" (get_current ()->addr_limit.seg)); flag;}
						     ) == 0))
				    do
				    {
				    __pu_err = 0;
...
__asm__ __volatile__ ("1:	movl %%eax,0(%2)\n" "2:	movl %%edx,4(%2)\n" "3:\n" ".section .fixup,\"ax\"\n" "4:	movl %3,%0\n" "	jmp 3b\n" ".previous\n" ".section __ex_table,\"a\"\n" "	.align 4\n" "	.long 1b,4b\n" "	.long 2b,4b\n" ".previous": "=r" (__pu_err): "A" (((__typeof__ (*(optval))) (rval))), "r" (__pu_addr), "i" (-14), "0" (__pu_err)); break; default:
				...
				static __inline__ void sockfd_put (struct
								   socket
								   *sock)
				{
				  fput (sock->file);
				...
				int trickles_sendv_impl (int fd,
							 struct cminisock
							 *msk,
							 struct tiovec
							 *user_tiov,
							 int tiovlen)
				{
				  ...
				  if (tiov_size > 0)
				      ...
				      if (tiov == ((void *) 0))
					{
				      ...
				      err = -14;
				      if ((__builtin_constant_p (tiov_size) ?
					   __constant_copy_from_user ((tiov),
								      (user_tiov),
								      (tiov_size))
					   : __generic_copy_from_user ((tiov),
								       (user_tiov),
								       (tiov_size))))
					{
					  goto out_freeiov;
				  ...
				  struct cminisock *api_msk =
				    allocate_api_msk (sk);
				  unmarshallContinuationServerPMSK2MSK (sk,
									api_msk,
									pmsk);

				  err =
				    trickles_do_sendmsg_tiov (sk, api_msk,
							      tiov, tiovlen);
				...
				static int
				  skb_fillpage_actor (read_descriptor_t *
						      ...
						      unsigned long size)
				{
				  ...
				  if (i >= 6)
				    ...
				    {
				      atomic_inc (&(page)->count);
				      fill_page_desc (skb, i, page, offset,
						      size);
				      written = size;
				      skb->len += size;
				      skb->data_len += size;
				...
				static ssize_t file_insertpages (struct
								 sk_buff *skb,
								 int in_fd,
								 loff_t
								 offset,
								 size_t count)
				{
				  ...
				  if (!in_inode->i_mapping->a_ops->readpage)
				    goto fput_in;
				  retval =
				    locks_verify_area (1, in_inode, in_file,
						       in_file->f_pos, count);
				  if (retval)
				    goto fput_in;
				  ...
				  if (count)
				    {
				      ...
				      desc.buf = (char *) skb;
				      desc.error = 0;
				      do_generic_file_read (in_file, &offset,
							    &desc,
							    skb_fillpage_actor);
				...
				static inline int fiov_handler (struct sock
								*origSK,
								struct
								cminisock
								*msk,
				  ...
				  for (i = 0; i < msk->num_packets; i++)
				      ...
				      while (fiov_pos < fiovlen)
					{
					  ...
					  if (fiov[fiov_pos].fd == -2)
						...
						{
						  printk
						    ("cannot copy; after first page fragment of packet\n");
						  kfree_skb (skb);
						  goto out;
						}

					      if (fiov[fiov_pos].offset +
						  fiov_offs >=
						  (0x1ULL << 32ULL))
						{
						  printk
						    ("trickles does not handle file offsets > 32 bits\n");
						}
					      char *src =
						(char *) fiov[fiov_pos].
						offset + fiov_offs;
					      err =
						skb_add_data (skb, src, amt);
					      if (err)
						{
						  printk
						    ("error while csum/copy, base = %p amt = %d pkt_type = %d\n",
						     src, amt,
						     msk->packets[i].type);
						  kfree_skb (skb);
						  rval = -22;
					      ...
					      if (res != amt)
						{
						  rval = res;
						  printk
						    ("insufficient file data for insertion\n");
						  goto out;
						}
					    }
				      ...
				      if (userapi_pkt_spew)
					{
					  printk ("sending pkts[%d] %u-%u\n",
						  i, pkts[i].seq,
						  pkts[i].seq + pkts[i].len);
					}

				      if (totallen == 0 &&
					  fiov[fiovlen - 1].offset == 0
					  && fiov[fiovlen - 1].len == 0)
					{
					  msk->tag = FIN;
					}
				...
				int trickles_sendfilev_impl (int fd,
							     struct cminisock
							     *msk,
							     struct fiovec
							     *user_fiov,
							     int fiovlen)
				{
				    ...
				    }
				  {
					...
					err = -14;
					if ((__builtin_constant_p (fiov_size)
					     ?
					     __constant_copy_from_user ((fiov), (user_fiov), (fiov_size)) : __generic_copy_from_user ((fiov), (user_fiov), (fiov_size))))
					  {
					    goto out_freefiov;
					  }
				      }
				  ...
				  err =
				    trickles_do_sendmsg_fiov (sk, api_msk,
							      fiov, fiovlen);
				...
				struct tiovec iov = { iov_base: buf, iov_len:len,
				  ...
				  if (iov.iov_len == -1)
				    ...
				    more:;
				      ...
				      struct cminisock *api_msk =
					allocate_api_msk (sk);
				      unmarshallContinuationServerPMSK2MSK
					(sk, api_msk, pmsk);
				...
				int trickles_setucont_impl (int fd,
							    struct cminisock
							    *msk_compat,
							    int pkt_num,
							    char *user_ucont,
							    ...
							    ucont_len)
				{
				  ...
				  printk
				    ("pmsk num_packet = %d, msk num_packet = %d\n",
				     pmsk->num_packets,
				     msk_compat->num_packets);

				  if (pkt_num < 0
				      || pkt_num >= pmsk->num_packets)
				    {
				  ...
				  if (ucont_len > 0)
				      ...
				      if (buf == ((void *) 0))
					{
				      ...
				      if ((__builtin_constant_p (ucont_len) ?
					   __constant_copy_from_user ((buf),
								      (user_ucont),
								      (ucont_len))
					   : __generic_copy_from_user ((buf),
								       (user_ucont),
								       (ucont_len))))
					{
					  printk
					    ("set_ucont: fault while copying\n");
					  err = -14;
					  do
					    {
					      do
				  ...
				  setPacketUCont (&pmsk->packets[pkt_num],
						  buf, ucont_len);
				  err = 0;
				  do
				    {
				...
				static int trickles_write_memory_free (struct
								       sock
								       *sk)
				{
				  struct tcp_opt *tp = &sk->tp_pinfo.af_tcp;
				  return tp->t.write_seq - tp->t.snd_una <
				    sk->sndbuf;
				}
				...
				static int wait_for_write_memory (struct sock
								  *sk,
								  long *timeo)
				{
				...
				int trickles_client_sendmsg (struct sock *sk,
							     struct msghdr
							     *msg, int size)
				{

				  int i;
				  int iovlen, totalLen = 0, skb_spaceleft =
				    0, result = 0, position = 0;
					  ...
					  if (skb_spaceleft == 0)
					      ...
					      if (skb != ((void *) 0))
						{
						  ...
						  ((struct tcp_skb_cb *)
						   &((skb)->cb[0]))->end_seq =
			      tp->t.write_seq;
						  __skb_queue_tail (&tp->t.
								    requestBytes,
								    skb);
						}
					      if (!
						  ((sk)->tp_pinfo.af_tcp.t.
						   write_seq <
						   (sk)->tp_pinfo.af_tcp.t.
						   snd_una + (sk)->sndbuf))
						{
						   ...
						   sk->sndbuf))
						{
						  printk
						    ("KERNEL: assertion ("
						     "tp->t.write_seq - tp->t.snd_una <= sk->sndbuf"
						     ") failed at "
						     "trickles-userapi.c"
						     "(%d)\n", 1824);
						};
					      ...
					      skb =
						alloc_skb (skb_spaceleft,
							   (0x20));
					      if (skb == ((void *) 0))
						{
						  printk
						    ("Trickles_client_sendmsg: out of memory\n");
						  goto sleep;
						}
					      ((struct tcp_skb_cb *)
					       &((skb)->cb[0]))->seq =
			  tp->t.write_seq;
					  ...
					  if ((__builtin_constant_p (copyLen)
					       ?
					       __constant_copy_from_user ((buf), (iov[i].iov_base + iov_position), (copyLen)) : __generic_copy_from_user ((buf), (iov[i].iov_base + iov_position), (copyLen))))
					    {
					      printk
						("Trickles_client_sendmsg: fault while copying from client\n");
					      result = -14;
					      goto out_dealloc;
					    }
				  ...
				  if (skb != ((void *) 0))
				    {
				      ...
				      ((struct tcp_skb_cb *)
				       &((skb)->cb[0]))->end_seq =
		  tp->t.write_seq;
				      __skb_queue_tail (&tp->t.requestBytes,
							skb);
				    }
				...
				void trickles_syn_piggyback_impl (struct sock
								  *sk,
								  struct
								  sk_buff
								  *skb)
				{
				  ...
				  struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
				  int sendBufLen =
				    tp->t.write_seq - tp->t.snd_una;

				  if (sendBufLen > 0)
				    {
				      int copyLen =
					({ typeof (sendBufLen) Z =
					 (typeof (sendBufLen)) ((sysctl_trickles_mss) - (128 + 32));
					 (
					   {
					   const typeof (sendBufLen) _x =
					   (sendBufLen);
					   const typeof (Z) _y = (Z);
					   (void) (&_x == &_y);
					   _x < _y ? _x : _y;
					...
					tp->t.requestBytes.next;
				      int csum = 0;
				      while (copyLen > 0)
					{
					  if (!
					      ((void *) input_skb !=
					       (void *) &tp->t.requestBytes))
					    {
					      printk ("KERNEL: assertion ("
						      "(void*)input_skb != (void*)&tp->t.requestBytes"
						      ") failed at "
						      "trickles-userapi.c"
						      "(%d)\n", 1902);
					    };
					  unsigned pieceLen =
					    ({ typeof (input_skb->len) Z =
					     (typeof (input_skb->len))
					     (copyLen);
					     (
					       {
					       const typeof (input_skb->
							     len) _x =
					       (input_skb->len);
					       const typeof (Z) _y = (Z);
					       (void) (&_x == &_y);
					  ...
					  if (srcErr || dstErr)
					    {
					      printk
						("error while piggybacking data on SYN\n : %d %d\n",
						 srcErr, dstErr);
					    }
					  copyLen -= pieceLen;
				...
				static long tcp_data_wait (struct sock *sk,
							   long timeo)
				{
				wait_queue_t wait = { task: get_current (), task_list:{((void *) 0), ((void *) 0)},
				  };
							     ...
							     int *addr_len)
				{
				  ...
				  target =
				    sock_rcvlowat (sk, flags & 0x100, len);
				...
				static inline void dump_tiov (struct tiovec
							      *tiov)
				{
				  printk ("tiov_base = %p\ntiov_len = %d",
				...
				static inline void dump_mskdesc (struct
								 mskdesc
								 *mskd)
				{
				  printk ("msk = %p, num =  %d\n", mskd->msk,
				...
				int trickles_sendbulk_impl (int fd,
							    struct mskdesc
							    *user_descbuf,
							    int descbuf_len)
				{
				  ...
				  if (descbuf == ((void *) 0))
				    {
				      printk
					("trickles_sendbulk_impl: user desc buf out of memory\n");
				      goto out_put;
				    }
				  if ((err =
				       (__builtin_constant_p (descbuf_len) ?
					__constant_copy_from_user ((descbuf),
								   (user_descbuf),
								   (descbuf_len))
					: __generic_copy_from_user ((descbuf),
								    (user_descbuf),
								    (descbuf_len)))))
				    {
				      printk
					("trickles_sendbulk_impl: copy_from_user returned %d\n",
					 err);
				      err = -14;
				      goto out_dealloc;
				    }
				  ...
				  while (curr_desc < (struct mskdesc *) tail)
					...
					{
					  printk
					    ("trickles_sendbulk_impl: bad minisocket %d %d @ %d\n",
					     !(((((char *) (msk) -
						  (char *) ((tp)->
							    cminisock_api_config.
							    cfg.ctl->
				...
				int trickles_read_proc (char *page,
							char **start,
							off_t offset,
							int count, int *eof,
							void *data)
				{
				  ...
				  for (logEntry =
				       (typeof (logEntry))
				       (&tricklesProcLogHead)->next;
				       (logEntry !=
					(typeof (logEntry))
					(&tricklesProcLogHead));
				       logEntry =
				       (typeof (logEntry)) logEntry->next)
				    {
				      char tempBody[4096], *temp = tempBody;
				      int first =
					logEntry->returnedEvents == 0;
				      if (first)
					{
					  int count = 0;
					  for (i = logEntry->returnedEvents;
					       i < logEntry->size; i++)
					    {
					  ...
					  sprintf (temp,
						   "Socket (daddr=%X port=%d) (rcv_nxt=%u,trcv_nxt=%u) [%d]\n",
						   htonl (logEntry->addr),
						   htons (logEntry->port),
						   logEntry->rcv_nxt,
						   logEntry->t_rcv_nxt,
					   ...
					   i < logEntry->size; i++)
					{
					  ...
					  sprintf (temp,
						   "%s: cwnd=%d state=%s time=%d\n",
						   saveLocation,
						   (int) event->cwnd, state,
						   event->time);
					  if (first)
					    {
					      temp = tempBody;
					  ...
					  int copyLen =
					    ({
					     typeof (({typeof
							...
							((int)
							 ...
							 _x < _y ? _x : _y;
									  ...
									  {
									  typeof
									  ((int) strlen (temp)) Z = (typeof ((int) strlen (temp))) (count - pos);
									  (
									    {
									    const
									    typeof
									    ((int)strlen (temp)) _x = ((int)strlen (temp));
									    const
									    typeof
									    (Z)
									    _y
					     ...
					     (__builtin_constant_p (copyLen) ?
					      __constant_memcpy ((page + pos),
								 (temp),
								 (copyLen)) :
					      __memcpy ((page + pos), (temp),
							(copyLen)));
					     pos += copyLen; if (done)
					     {
					     ...
					     *eof = 0;}
					     do
					     {
				  unsigned int *ptr = &((void)((0)), irq_stat[0].__local_bh_count); __asm__ __volatile__ ("": : : "memory"); if (!--*ptr) __asm__ __volatile__ ("cmpl $0, -8(%0);" "jnz 2f;" "1:;" ".subsection 1\n\t" "" ".ifndef " ".text.lock." "tmalloc" "\n\t" ".text.lock." "tmalloc" ":\n\t" ".endif\n\t" "2: pushl %%eax; pushl %%ecx; pushl %%edx;" "call %c1;" "popl %%edx; popl %%ecx; popl %%eax;" "jmp 1b;" ".previous\n\t": :"r" (ptr), "i" (do_softirq));}
					     while (0);
					     *start = page; return pos;}

					     struct alloc_head_list
					     tricklesCwndProcLogHead;
					     ...
					     (__builtin_constant_p (copyLen) ?
					      __constant_memcpy ((bb->dest +
								  prevPos),
								 (data),
								 (copyLen)) :
					      __memcpy ((bb->dest + prevPos),
							(data), (copyLen)));
					     return copyLen;}

					     spinlock_t cwndLogLock =
					     (spinlock_t)
					     {
					     };
					     void trickles_logCwnd_impl (enum
									 LogCwndType
									 type,
									 int
									 addr,
					     ...
					     int trickles_cwnd_read_proc (char
									  *page,
									  char
									  **start,
									  off_t
					     ...
					     do
					     {
				    ((void) ((0)), irq_stat[0].__local_bh_count)++; __asm__ __volatile__ ("": : :"memory");}
					     while (0);
					     for (logEntry =
						  (typeof (logEntry))
						  (&tricklesCwndProcLogHead)->
...
sprintf (tempbuf, "cont - %d:%d = %d/%d/%d TCPBase: %d @ %d s %d us rtt1: %d \n", logEntry->seq, logEntry->ack_seq, logEntry->startCwnd, logEntry->effCwnd, logEntry->ssthresh, logEntry->rtt, logEntry->s, logEntry->us, logEntry->srtt); break; case EVENT_RECORD:
					     ...
					     {
case (1):
eventTypeStr = "Recovery"; break; case (2):
eventTypeStr = "Timeout0"; break; case (6):
eventTypeStr = "Timeout1"; break; case (3):
eventTypeStr = "UserBlock"; break; case (4):
eventTypeStr = "UserUnblock"; break; case (5):
eventTypeStr = "UserBadUnblock"; break; case (7):
eventTypeStr = "InSeq"; break; default:
					     eventTypeStr = "UNKNOWN";}
					     sprintf (tempbuf,
						      "%s:%d(%d-%d) = %d/%d/%d TCPBase: %d @ %d s %d us\n",
						      eventTypeStr,
						      logEntry->ack_seq,
						      logEntry->addr,
						      logEntry->port,
...
default:
					     ...
					     char *src =
					     tempbuf + logEntry->sentAmount;
					     real_wrlen =
					     bb_write (&bb, src, wrlen =
						       strlen (src));
					     logEntry->sentAmount =
					     real_wrlen;
					     if (real_wrlen < wrlen)
				    ...
				    unsigned int *ptr = &((void)((0)), irq_stat[0].__local_bh_count); __asm__ __volatile__ ("": : : "memory"); if (!--*ptr) __asm__ __volatile__ ("cmpl $0, -8(%0);" "jnz 2f;" "1:;" ".subsection 1\n\t" "" ".ifndef " ".text.lock." "tmalloc" "\n\t" ".text.lock." "tmalloc" ":\n\t" ".endif\n\t" "2: pushl %%eax; pushl %%ecx; pushl %%edx;" "call %c1;" "popl %%edx; popl %%ecx; popl %%eax;" "jmp 1b;" ".previous\n\t": :"r" (ptr), "i" (do_softirq));}
					     ...
					     *start = page; return bb.curPos;}
					     ...
					     int limit;};
					     static inline
					     void *lbb_get_end (struct
								linear_bounded_buffer
								*lbb)
					     {
					     return lbb->start + lbb->limit;}

					     static inline
					     int lbb_get_offset (struct
								 linear_bounded_buffer
								 *lbb)
					     {
					     ...
					     static inline
					     void *lbb_reserve (struct
								linear_bounded_buffer
								*lbb, int len)
					     {
					     char *rval;
					     rval = lbb->curr;
					     if (!lbb_check_reserve
						 (lbb, len))
					     {
					     return ((void *) 0);}
					     lbb->curr += len; return rval;}

					     static inline
					     void *lbb_append (struct
							       linear_bounded_buffer
							       *lbb,
							       void *data,
							       int len)
					     ...
					     (__builtin_constant_p (len) ?
					      ...
					      ((dest), (data),
					       (len)) :
					      __generic_copy_to_user_nocheck ((dest), (data), (len))); return dest;}
					     ...
					     static inline void
					     copy_patch_hmac_update_in (struct
									HMAC_CTX
									*hmac_ctx,
									void
									*user_addr,
					     ...
					     (__builtin_constant_p
					      ...
					      __constant_copy_to_user_nocheck
					      ((user_addr + hmacStart),
					       (kernel_addr + hmacStart),
					       (hmacEnd -
						hmacStart)) :
					      __generic_copy_to_user_nocheck ((user_addr + hmacStart), (kernel_addr + hmacStart), (hmacEnd - hmacStart))); hmac_update (hmac_ctx, (char *) kernel_addr + hmacStart, hmacEnd - hmacStart); int zero = 0; if (!(patch.width <= sizeof (zero)))
					     {
					     printk ("KERNEL: assertion ("
						     "patch.width <= sizeof(zero)"
						     ") failed at "
						     "trickles-userapi.c"
						     "(%d)\n", 2508);};
					     (__builtin_constant_p
					      (patch.
					       width) ?
					      __constant_copy_to_user_nocheck
					      ((user_addr + patch.offset),
					       (&zero),
					       (patch.
						width)) :
					      __generic_copy_to_user_nocheck ((user_addr + patch.offset), (&zero), (patch.width))); hmac_update (hmac_ctx, &zero, patch.width); lastOffset = patch.offset + patch.width;}}
					     else
					     {
					     (__builtin_constant_p (len) ?
					     ...
					     {
					     printk ("KERNEL: assertion ("
						     "patch.offset + patch.width <= len"
						     ") failed at "
						     "trickles-userapi.c"
						     "(%d)\n", 2508);};
					     if (!
						 (__builtin_memcmp
						  (kernel_addr + patch.offset,
						   &zero, patch.width) == 0))
					     {
					     printk ("KERNEL: assertion ("
						     "memcmp(kernel_addr + patch.offset, &zero, patch.width) == 0"
						     ") failed at "
						     "trickles-userapi.c"
						     "(%d)\n", 2508);};
					     (__builtin_constant_p (0)
						 ...
						 (((kernel_addr +
						  ...
						  ((patch.
						    width))) :
						 __constant_c_memset (((kernel_addr + patch.offset)), ((0x01010101UL * (unsigned char) (0))), ((patch.width)))) : (__builtin_constant_p ((patch.width)) ? __memset_generic ((((kernel_addr + patch.offset))), (((0))), (((patch.width)))) : __memset_generic (((kernel_addr + patch.offset)), ((0)), ((patch.width)))));}} if (!(len >= lastOffset))
					     {
					     printk ("KERNEL: assertion ("
						     "len >= lastOffset"
						     ") failed at "
					     ...
					     (__builtin_constant_p
					      (len -
					       lastOffset) ?
					      __constant_copy_to_user_nocheck
					      ((user_addr + lastOffset),
					       (kernel_addr + lastOffset),
					       (len -
						lastOffset)) :
					      __generic_copy_to_user_nocheck ((user_addr + lastOffset), (kernel_addr + lastOffset), (len - lastOffset))); if ((in == out))
					     {
					     hmac_update (hmac_ctx,
							  ...
							  lastOffset);}};
					     static inline void
					     copy_patch_hmac_update_out
					     (struct HMAC_CTX *hmac_ctx,
					      void *user_addr,
					      void *kernel_addr, int len,
					      void (*const generator) (const
								       void
								       *const
								       context,
								       int
						...
						width)) :
					     ...
					     (__builtin_constant_p
					      ...
					      ((user_addr + lastOffset),
					       ...
					       (len -
						lastOffset)) :
					      __generic_copy_to_user_nocheck ((user_addr + lastOffset), (kernel_addr + lastOffset), (len - lastOffset))); if ((out == out))
					     {
					     hmac_update (hmac_ctx,
							  (char *) kernel_addr
							  + lastOffset,
							  ...
							  lastOffset);}};
					     static inline void
					     arrayPatchGenerator (const void
								  *const
								  context,
								  int num,
								  struct
								  ...
								  *patch)
					     {
					     struct patch_desc * array =
					     (struct patch_desc *)context;
					     *patch = array[num];}

					     static inline void
					     packetPatchGenerator (const void
								   *const
								   context,
								   int num,
								   struct
						...
						0)->ucontData));}
					     ...
					     void DeferralContext_init (struct
									DeferralContext
									*ctx)
					     {
					     ctx->numDeferrals = 0;
					     ...
					     void DeferralContext_free (struct
									DeferralContext
									*ctx)
					     {
					     if (ctx->next != ((void *)0))
					     ...
					     int defer (struct DeferralContext
							*ctx, int value)
					     {
					     ...
					     {
					     printk ("KERNEL: assertion ("
						     "ctx->numDeferrals == MAX_NUM_DEFERRALS_PER_CONTEXT"
						     ") failed at "
						     "trickles-userapi.c"
						     "(%d)\n", 2543);};
					     ctx = ctx->next;}
					     ...
					     {
					     printk
					     ("Out of memory while allocating deferral spillb lock\n");
					     return -1;}
					     ctx = ctx->next;}
					     ctx->data[ctx->numDeferrals++] =
					     value; return 0;}


					     void apply_deferrals (struct
								   HMAC_CTX
								   *hmac_ctx,
								   struct
								   DeferralContext
					     ...
					     ((void *)0);
					     static inline void
					     *tmalloc_helperin (struct sock
								*sk, int len)
					     {
					     printk
					     ("tmalloc_helperin'ing %d\n",
					      len); return tmalloc (sk, len);}

					     static inline void
					     *kmalloc_helperin (struct sock
								*sk, int len)
					     {
					     ...
					     {
					     printk ("in"
						     ": Result overflow on currStart or output_msk\n");
					     err = (-1); goto convert_error;}
					     if ((in == out))
					     {
					     (
					       {
...
__asm__ __volatile__ ("1:	mov" "l" " %2,%" "" "1\n" "2:\n" ".section .fixup,\"ax\"\n" "3:	movl %3,%0\n" "	xor" "l" " %" "" "1,%" "" "1\n" "	jmp 2b\n" ".previous\n" ".section __ex_table,\"a\"\n" "	.align 4\n" "	.long 1b,3b\n" ".previous": "=r" (__gu_err), "=r" (__gu_val): "m" ((*(struct __large_struct *) (((len))))), "i" (-14), "0" (__gu_err)); break; default:
					     ...
					     const struct patch_desc const
					     mskPatchArray[] =
					     {
					     {
									    offset: (((char *)&((struct cminisock *)0)->sk) - (char *)((void *)0)), width:(sizeof
					      (((struct cminisock *) 0)->
					       sk))},
					     {
									    offset: (((char *)&((struct cminisock *)0)->ucont_data) - (char *)((void *)0)), width:(sizeof
					      (((struct cminisock *) 0)->
					       ucont_data))},
					     {
									    offset: (((char *)&((struct cminisock *)0)->input) - (char *)((void *)0)), width:(sizeof
					      (((struct cminisock *) 0)->
					       input))},
					     {
									  offset: (((char *)&((struct cminisock *)0)->packets) - (char *)((void *)0)), width:(sizeof
					      (((struct cminisock *) 0)->
					       packets))}};;
					     copy_patch_hmac_update_in
					     (hmac_ctx, user_msk, msk,
					      sizeof (*msk),
					      arrayPatchGenerator,
					      mskPatchArray,
					      (sizeof (mskPatchArray) /
					       sizeof (mskPatchArray[0]))); (
										       {
										       if
										       ((msk->ucont_len) == 0)
										       {
											...
											((void *) 0), ((void *) 0), 0); if ((in == out)) (
																		 ...
																		 0;
																		 switch
																		 ((sizeof (*(&((struct cminisock *) _tempdest)->ucont_data))))
																		 {
case 1:
...
__asm__ __volatile__ ("1:	movl %%eax,0(%2)\n" "2:	movl %%edx,4(%2)\n" "3:\n" ".section .fixup,\"ax\"\n" "4:	movl %3,%0\n" "	jmp 3b\n" ".previous\n" ".section __ex_table,\"a\"\n" "	.align 4\n" "	.long 1b,4b\n" "	.long 2b,4b\n" ".previous": "=r" (__pu_err): "A" (((__typeof__ (*(&((struct cminisock *) _tempdest)->ucont_data))) ((char *) _tempdest - currStart))), "r" (((&((struct cminisock *) _tempdest)->ucont_data))), "i" (-14), "0" (__pu_err)); break; default:
						   ...
						   if ((in == out)) (
									   ...
									   0;
									   switch
									   ((sizeof (*(&((struct cminisock *) _tempdest)->input))))
									   {
case 1:
...
__asm__ __volatile__ ("1:	movl %%eax,0(%2)\n" "2:	movl %%edx,4(%2)\n" "3:\n" ".section .fixup,\"ax\"\n" "4:	movl %3,%0\n" "	jmp 3b\n" ".previous\n" ".section __ex_table,\"a\"\n" "	.align 4\n" "	.long 1b,4b\n" "	.long 2b,4b\n" ".previous": "=r" (__pu_err): "A" (((__typeof__ (*(&((struct cminisock *) _tempdest)->input))) ((char *) _tempdest - currStart))), "r" (((&((struct cminisock *) _tempdest)->input))), "i" (-14), "0" (__pu_err)); break; default:
					     ...
					     (
						   ...
						   err = (-1);
						   goto convert_error;}
						   msk->packets = alloc_ptr;}
						   copy_patch_hmac_update_in
						   (hmac_ctx, _tempdest,
						    (msk->packets), _len,
						    packetPatchGenerator,
						    ((void *) 0),
						    msk->num_packets);
						   if ((in == out)) (
									    {
									    ...
									    0;
									    switch
									    ((sizeof (*(&((struct cminisock *) _tempdest)->packets))))
									    {
case 1:
...
__asm__ __volatile__ ("1:	movl %%eax,0(%2)\n" "2:	movl %%edx,4(%2)\n" "3:\n" ".section .fixup,\"ax\"\n" "4:	movl %3,%0\n" "	jmp 3b\n" ".previous\n" ".section __ex_table,\"a\"\n" "	.align 4\n" "	.long 1b,4b\n" "	.long 2b,4b\n" ".previous": "=r" (__pu_err): "A" (((__typeof__ (*(&((struct cminisock *) _tempdest)->packets))) ((char *) _tempdest - currStart))), "r" (((&((struct cminisock *) _tempdest)->packets))), "i" (-14), "0" (__pu_err)); break; default:
						...
						((void *) 0), 0);
					       if ((in == out)) (
								       ...
								       = 0;
								       switch
								       ((sizeof (*(&((struct cminisock_packet *) _tempdest)->ucontData))))
								       {
case 1:
...
__asm__ __volatile__ ("1:	movl %%eax,0(%2)\n" "2:	movl %%edx,4(%2)\n" "3:\n" ".section .fixup,\"ax\"\n" "4:	movl %3,%0\n" "	jmp 3b\n" ".previous\n" ".section __ex_table,\"a\"\n" "	.align 4\n" "	.long 1b,4b\n" "	.long 2b,4b\n" ".previous": "=r" (__pu_err): "A" (((__typeof__ (*(&((struct cminisock_packet *) _tempdest)->ucontData))) ((char *) _tempdest - currStart))), "r" (((&((struct cminisock_packet *) _tempdest)->ucontData))), "i" (-14), "0" (__pu_err)); break; default:
					     ...
					     {
					     int check_len =
					     lbb_get_offset (lbb) - startLen;
					     if (!(desc_len == check_len))
					     {
					     if (!(desc_len == check_len))
					     {
					     printk ("KERNEL: assertion ("
...
__asm__ __volatile__ ("1:	mov" "l" " %2,%" "" "1\n" "2:\n" ".section .fixup,\"ax\"\n" "3:	movl %3,%0\n" "	xor" "l" " %" "" "1,%" "" "1\n" "	jmp 2b\n" ".previous\n" ".section __ex_table,\"a\"\n" "	.align 4\n" "	.long 1b,3b\n" ".previous": "=r" (__gu_err), "=r" (__gu_val): "m" ((*(struct __large_struct *) (((len))))), "i" (-14), "0" (__gu_err)); break; default:
					     ...
					     (*count)++; if (0)
					     {
													    convert_error:if (deferral_ctx.next !=
						 ((void *) 0))
					     DeferralContext_free
					     (deferral_ctx.next); return -22;}
					     return 0;};
					     static
					     __attribute__ ((always_inline))
					     int convertMSK_out (struct sock
								 *sk,
								 struct
								 HMAC_CTX
								 *hmac_ctx,
...
__asm__ __volatile__ ("1:	movl %%eax,0(%2)\n" "2:	movl %%edx,4(%2)\n" "3:\n" ".section .fixup,\"ax\"\n" "4:	movl %3,%0\n" "	jmp 3b\n" ".previous\n" ".section __ex_table,\"a\"\n" "	.align 4\n" "	.long 1b,4b\n" "	.long 2b,4b\n" ".previous": "=r" (__pu_err): "A" (((__typeof__ (*(&((struct cminisock *) _tempdest)->ucont_data))) ((char *) _tempdest - currStart))), "r" (((&((struct cminisock *) _tempdest)->ucont_data))), "i" (-14), "0" (__pu_err)); break; default:
						   ...
						   ("result overflow - %s\n",
						    _dest_name); err = (-1);
						   goto convert_error;}
						   if (tmalloc_helperout !=
						       ((void *) 0))
						   {
						   void *alloc_ptr =
						   tmalloc_helperout (sk,
								      msk->
								      input_len);
						   if (alloc_ptr ==
						       ((void *) 0))
...
__asm__ __volatile__ ("1:	movl %%eax,0(%2)\n" "2:	movl %%edx,4(%2)\n" "3:\n" ".section .fixup,\"ax\"\n" "4:	movl %3,%0\n" "	jmp 3b\n" ".previous\n" ".section __ex_table,\"a\"\n" "	.align 4\n" "	.long 1b,4b\n" "	.long 2b,4b\n" ".previous": "=r" (__pu_err): "A" (((__typeof__ (*(&((struct cminisock *) _tempdest)->packets))) ((char *) _tempdest - currStart))), "r" (((&((struct cminisock *) _tempdest)->packets))), "i" (-14), "0" (__pu_err)); break; default:
						...
						_dest_name); err = (-1);
					       goto convert_error;}
					       if (kmalloc_helperout !=
						   ((void *) 0))
					       {
					       void *alloc_ptr =
					       kmalloc_helperout (sk,
								  msk->
								  packets[i].
								  ucontLen);
					       if (alloc_ptr == ((void *) 0))
...
__asm__ __volatile__ ("1:	mov" "l" " %2,%" "" "1\n" "2:\n" ".section .fixup,\"ax\"\n" "3:	movl %3,%0\n" "	xor" "l" " %" "" "1,%" "" "1\n" "	jmp 2b\n" ".previous\n" ".section __ex_table,\"a\"\n" "	.align 4\n" "	.long 1b,3b\n" ".previous": "=r" (__gu_err), "=r" (__gu_val): "m" ((*(struct __large_struct *) (((len))))), "i" (-14), "0" (__gu_err)); break; default:
					     ...
					     extract_MSKs_from_lbb (struct
								    ...
								    *lbb);
					     static inline void
					     msk_force_free (struct sock *sk,
							     struct cminisock
							     *msk)
					     {
					     printk
					     ("msk_force_free users not updated to pmsk\n");
					     do
					     {
					     if (!(0))
					     {
					     ...
					     int
					     trickles_extract_events_impl (int
									   fd,
									   struct
									   extract_mskdesc_in
									   *descbuf,
					     ...
					     show_stack (((void *) 0));
					     breakpoint ();}}
					     while (0); int hitFreeLoop = 0;
					     struct socket * sock;
					     struct sock * sk = ((void *)0);
					     struct tcp_opt * tp; int err;
					     int hmac_input_len;
					     if (descbuf_len <= 0)
					     {
					     err = -22; goto out;}
					     ...
					     msk_freelist.len; if (!((
									  {
																					unsigned long flag, sum; asm ("addl %3,%1 ; sbbl %0,%0; cmpl %1,%4; sbbl $0,%0": "=&r" (flag), "=r" (sum):"1" (descbuf), "g" ((int) (descbuf_len)), "g" (get_current ()->addr_limit.seg)); flag;}) == 0))
					     {
					     printk
					     ("descbuf not readable\n");
					     err = -14; goto out_put;}
					     if (!((
						     {
																					unsigned long flag, sum; asm ("addl %3,%1 ; sbbl %0,%0; cmpl %1,%4; sbbl $0,%0": "=&r" (flag), "=r" (sum):"1" (dest), "g" ((int)
								      (*destLen)),
									     "g"
									     (get_current
									      ()->
					     ...
					     =
					     {
																					start: (char *)dest, curr: (char *)dest, limit:*destLen};
					     struct extract_mskdesc_in *
					     currentInputDesc = descbuf;
					     struct msk_collection *
					     outputCollection = dest;
					     lbb_reserve (&lbb,
							  sizeof (struct
								  msk_collection));
					     char hmac[16];
					     ...
					     *tp->t.hmacCTX;
					     hmac_init (&hmac_ctx);
					     *destLen = 0; int numCopies = 0;
					     while ((char *) currentInputDesc
						    - (char *) descbuf <
						    descbuf_len)
					     {
					     struct extract_mskdesc_in
					     ...
					     mskdesc.msk; if (!(
									    ...
									    *
									    _tp
									    =
									    &((sk)->tp_pinfo.af_tcp); (((((char *) ((msk)) - (char *) ((_tp)->cminisock_api_config.cfg.ctl->minisock_base)) % sizeof (struct cminisock)) == 0) && (((void *) ((struct cminisock *) ((msk)))) >= ((void *) ((_tp)->cminisock_api_config.cfg.ctl->minisock_base)) && ((void *) ((struct cminisock *) ((msk)) + 1)) <= ((_tp)->cminisock_api_config.cfg.ctl->minisock_limit))) && (((msk))->ctl == ALLOC_READY || ((msk))->ctl == ALLOC_HALFFREE);}))
					     {
					     printk
					     ("Invalid msk in descbuf list, %d %d\n",
					      (((((char *) ((msk)) -
						  (char *) ((tp)->
							    cminisock_api_config.
							    cfg.ctl->
						...
						((tp)->cminisock_api_config.
						 cfg.ctl->minisock_limit)))
					      && (((msk))->ctl == ALLOC_READY
						  || ((msk))->ctl ==
						  ALLOC_HALFFREE)); err = -22;
					     goto out_put;}
					     if (msk->ctl != ALLOC_READY)
					     {
					     printk
					     ("Only alloc_ready packets are allowed to be extracted. alloc_halffree packets have no convenient access path on the destination\n");
					     err = -22; goto out_put;}
					     ...
					     {
					     printk
					     ("Better output error semantics (valid truncation) not implemented!\n");
					     goto out_put;}
					     if (!(err == (-2)))
					     {
					     printk ("KERNEL: assertion ("
					     ...
					     {
					     printk
					     ("Dealloc called on an msk that is not half-freed\n");
					     err = -22; goto out_put;}
					     free_trickles_msk_finish (sk,
								       msk);}
					     currentInputDesc =
					     ((currentInputDesc) + 1);}
																					goto truncated_output; truncated_output:

					     hmac_input_len = hmac_ctx.len;
					     hmac_final (&hmac_ctx, hmac);
					     (__builtin_constant_p (16) ?
					      __constant_copy_to_user_nocheck
					      ((outputCollection->hmac),
					       (hmac),
					       (16)) :
					      __generic_copy_to_user_nocheck ((outputCollection->hmac), (hmac), (16))); outputCollection->len = *destLen = lbb.curr - lbb.start;; printk ("Starting instant self-test\n"); int test_numIn; struct alloc_head_list new_msk_list; init_head (&new_msk_list); lbb.limit = lbb_get_offset (&lbb); lbb.curr = lbb.start; if (!(lbb_get_offset (&lbb) == 0))
					     {
					     printk ("KERNEL: assertion ("
						     "lbb_get_offset(&lbb) == 0"
						     ") failed at "
						     "trickles-userapi.c"
						     "(%d)\n", 2936);};
					     struct msk_collection *
					     ...
					     *tp->t.hmacCTX;
					     ...
					     hmac_update (&test_hmac_ctx,
							  hmac_start,
							  hmac_len);
					     hmac_final (&test_hmac_ctx,
							 test_hmac);
					     printk
					     ("instant Hmac computed on %d @%p\n",
					      hmac_len, hmac_start);
					     int memcmp_result =
					     __builtin_memcmp (test_hmac,
							       hmac, 16);
					     do
					     {
					     if ((memcmp_result) !=
						 (0)) printk ("memcmp_result"
							      " = %d, " "0"
							      " = %d\n",
							      (int)
							      (memcmp_result),
					     ...
					     if ((new_msk_list.len) !=
						 (test_numIn))
					     printk ("new_msk_list.len"
						     " = %d, " "test_numIn"
						     " = %d\n",
						     (int) (new_msk_list.len),
						     (int) (test_numIn));
					     else;
					     }
					     while (0);}
					     ...
					     {
					     struct cminisock * toFree =
					     (struct cminisock *)new_msk_list.
					     next;
					     if (!
						 (toFree !=
						  (struct cminisock *)
					     ...
					     {
					     printk ("KERNEL: assertion ("
						     "toFree != (struct cminisock*)&new_msk_list"
						     ") failed at "
						     "trickles-userapi.c"
						     "(%d)\n", 2976);};
					     ...
					     while (0);}
					     while (0);}
																						      while (0); sockfd_put (sock); out:
					     if (sk)
					     {
					     printk
					     ("FreeLoop(%d %d) After extract free list length %d=>%d\n",
					      hitFreeLoop, err, origFreeLen,
					      tp->cminisock_api_config.
					      msk_freelist.len);;}
					     return err;}

					     int
					     trickles_install_events_impl (int
									   fd,
									   struct
									   msk_collection
									   *descbuf,
					     ...
					     {
					     int remaining =
					     descbuf_len - (inputPosition -
							    (char *)descbuf);
					     printk
					     ("remaining = %d, %p %p\n",
					      remaining, inputPosition,
					      (char *) descbuf + descbuf_len);
					     struct linear_bounded_buffer lbb
					     =
					     {
																								      start: (char *)inputPosition, curr: (char *)inputPosition, limit:remaining};
					     struct msk_collection *
					     inputCollection =
					     (struct msk_collection *)
					     lbb_reserve (&lbb,
					     ...
					     {
					     printk
					     ("install_event -- descbuf too short for "
					      "inputCollection" "\n");
					     err = -22; goto out_put;}
					     }
					     while (0);
					     ...
					     {
					     printk ("KERNEL: assertion ("
						     "&inputCollection->descs[0] == lbb_get_pos(&lbb)"
						     ") failed at "
						     "trickles-userapi.c"
						     "(%d)\n", 3068);};
					     int collectionLen; (
...
__asm__ __volatile__ ("1:	mov" "l" " %2,%" "" "1\n" "2:\n" ".section .fixup,\"ax\"\n" "3:	movl %3,%0\n" "	xor" "l" " %" "" "1,%" "" "1\n" "	jmp 2b\n" ".previous\n" ".section __ex_table,\"a\"\n" "	.align 4\n" "	.long 1b,3b\n" ".previous": "=r" (__gu_err), "=r" (__gu_val): "m" ((*(struct __large_struct *) (((&inputCollection->len))))), "i" (-14), "0" (__gu_err)); break; default:
								       ...
								       while
								       (0);
								       ((collectionLen)) = (__typeof__ (*((&inputCollection->len)))) __gu_val; __gu_err;}
					     );
					     if (inputPosition +
						 collectionLen >
						 (char *) descbuf +
					     ...
					     {
					     printk
					     ("Not enough space for collection %d\n",
					      numCollections); err = -22;
					     goto out_put;}
					     lbb.limit = collectionLen;
					     int numIn; char input_hmac[16];
					     (__builtin_constant_p (16) ?
					      __constant_copy_from_user_nocheck
					      ((input_hmac),
					       (inputCollection->hmac),
					       (16)) :
					      __generic_copy_from_user_nocheck
					      ((input_hmac),
					       (inputCollection->hmac),
					     ...
					     *tp->t.hmacCTX; char hmac[16];
					     ...
					     hmac_final (&hmac_ctx, hmac);
					     if (__builtin_memcmp
						 (hmac, input_hmac, 16) != 0)
					     {
					     printk
					     ("input_events[%d]: Hmac verify failed\n",
					      numCollections); err = -22;
					     goto out_put;}
					     if ((numIn =
						  extract_MSKs_from_lbb (sk,
					     ...
					     {
					     printk ("KERNEL: assertion ("
						     "relink != (struct cminisock*)&new_msk_list"
						     ") failed at "
						     "trickles-userapi.c"
						     "(%d)\n", 3117);};
					     unlink ((struct alloc_head *)
					     ...
					     if (!((
								...
								*) ((struct
								     cminisock
								     *) ((relink)))) >= ((void *) ((_tp)->cminisock_api_config.cfg.ctl->minisock_base)) && ((void *) ((struct cminisock *) ((relink)) + 1)) <= ((_tp)->cminisock_api_config.cfg.ctl->minisock_limit))) && (((relink))->ctl == ALLOC_READY || ((relink))->ctl == ALLOC_HALFFREE);})))
					     {
					     printk ("KERNEL: assertion ("
						     "IS_VALID_MSK(sk, relink)"
						     ") failed at "
						     "trickles-userapi.c"
						     "(%d)\n", 3121);};
					     switch (relink->ctl)
					     {
case ALLOC_READY:
insert_tail (&tp->cminisock_api_config.cfg.ctl->msk_eventlist, (struct alloc_head *) relink); new_event (sk); break; case ALLOC_HALFFREE:
					     printk
					     ("No reliable access path to halffree msks!!!\n");
					     do
					     {
					     if (!(0))
					     {
					     ...
					     show_stack (((void *) 0));
					     breakpoint ();}}
while (0); break; default:
					     printk
					     ("Unhandled ctl state %d in installed event\n",
					      relink->ctl);
					     free_trickles_msk_finish (sk,
								       relink);
					     err = -22; goto out_put;}
						 ...
						 ((relink)->list ==
						  ...
						  || (relink)->list ==
						  &(tp)->cminisock_api_config.
						  cfg.ctl->msk_eventlist))
					     {
					     printk
					     ("Relink Msk %p not on valid list (list = %p)\n",
					      relink, relink->list);}
					     }
																									    err = totalNumIn; out_put:
					     do
					     {
					     do
					     {
					     ...
					     {
					     printk
					     ("Freeing msk still on new_msk_list %d\n",
					      new_msk_list.len);
					     while (new_msk_list.len > 0)
					     {
					     struct cminisock * toFree =
					     ...
					     {
					     printk
					     ("toFree Msk %p not on new_msk_list\n",
					      toFree);}
					     ...
					     {
					     printk ("KERNEL: assertion ("
						     "toFree->list == &tp->cminisock_api_config.msk_freelist"
						     ") failed at "
						     "trickles-userapi.c"
						     "(%d)\n", 3162);};}
					     }
					     printk ("Free list length %d\n",
						     ...
						     msk_freelist.len);
					     printk
					     ("After insert free list length %d=>%d\n",
					      origFreeLen,
					      tp->cminisock_api_config.
					      msk_freelist.len);;}
					     return err;}
					     ...
					     static inline
					     int extract_MSKs_from_lbb (struct
									sock
									*sk,
									struct
									alloc_head_list
					     ...
					     int err;
					     char *desc_end =
					     lbb_get_end (lbb); int numIn = 0;
					     while ((char *) lbb_get_pos (lbb)
						    < desc_end)
					     {
					     ...
					     int foo;
					     struct cminisock * msk =
					     alloc_trickles_msk (sk);
					     if (msk == ((void *) 0))
					     {
					     printk
					     ("could not allocate msk\n");
					     err = -12; goto out_put;}
					     if (convertMSK_in
						 (sk, ((void *) 0), msk, lbb,
						  &foo) != 0)
					     ...
					     {
					     printk
					     ("actual end of descbuf does not match param passed by client: %p %p\n",
					      actual_end, desc_end);}
																											  return numIn; out_put:
					     return err;}

					     static inline int
					     analyze_msk_list_helper (struct
								      sock
								      *sk,
								      int
								      print)
					     ...
					     {
case ALLOC_FREE:
ctlName = "ALLOC_FREE"; break;; case ALLOC_READY:
ctlName = "ALLOC_READY"; break;; case ALLOC_PENDING:
ctlName = "ALLOC_PENDING"; break;; case ALLOC_PROCESSING:
ctlName = "ALLOC_PROCESSING"; break;; case ALLOC_HALFFREE:
					     ctlName = "ALLOC_HALFFREE";
					     break;;}
					     if (print)
					     printk
					     ("MSK(%p %s) is on %s (%p)\n",
					      msk, ctlName, listName,
					      msk->list);}
					     }
					     return numInvalid;}

					     void analyze_msk_list (struct
								    sock *sk)
					     {
