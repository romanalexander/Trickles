...
static inline int
list_integrityCheck (struct alloc_head_list *list)
{
  int count = 0;
...
int trickles_cwnd_read_proc (char *page, char **start, off_t offset,
			     int count, int *eof, void *data);
extern void (*trickles_logCwnd_hook) (enum LogCwndType type, int addr,
				      int port, int seq, int ack_seq,
				      int startCwnd, int effCwnd,
...
int pminisock_insert (struct pminisock *msk);
	...
	if (1)
	  {
	      ...
	      if (0)
		{
		  ...
		  static inline void free_trickles_msk (struct sock *sk,
							struct cminisock
							*msk);
		  static inline void free_trickles_msk_finish (struct sock
							       *sk,
							       struct
							       cminisock
							       *msk);
		  struct list_link static struct cminisock
		    *shallow_copy_msk (struct sock *sk,
				       struct cminisock *pmsk)
		  {
		    struct cminisock *rval = alloc_trickles_msk (sk);
		    struct alloc_list_head head;
		    if (rval == ((void *) 0))
		      {
			printk ("out of memory while copying msk\n");
			return ((void *) 0);
		      }
		    list_link = *(struct alloc_list_head *) rval;
		    *rval = *pmsk;
		    *(struct alloc_list_head *) rval = head;
		    rval->refCnt = 1;
		    rval->isStatic = 0;
		    return rval;
		  }
		  ...
		  {;
		      ...
		      if (rval == ((void *) 0))
			{
		      ...
		      list_link = *(struct list_link *) rval;
		      *rval = *pmsk;
		    ...
		    static struct pminisock *copy_pmsk (struct sock *sk,
							...
							*pmsk)
		    {;
		      inline void pminisock_cache_child (struct pminisock
							 *msk,
							 int packet_number,
							 int flags);
			...
			return skb;
		      ...
		      int trickles_sendmsg (struct sock *sk,
			...
			atomic_t update_since_poll;
		      ...
		      extern int (*trickles_send_hook) (int fd,
							...
							msk, char *buf,
							int len);
		      int trickles_send_default (int fd,
						 struct cminisock *msk,
						 char *buf, int len);
		      extern int (*trickles_sendv_hook) (int fd,
							 ...
							 struct tiovec * tiov,
							 int tiovlen);
		      int trickles_sendv_default (int fd,
						  struct cminisock *msk,
						  struct tiovec *tiov,
						  int tiovlen);
		      ...
		      int trickles_mmap_default (struct file *file,
						 struct socket *sock,
						 struct vm_area_struct *vma);
		      extern int (*trickles_sock_poll_hook) (struct file *
							     file,
							     struct socket *
							     sock,
							     poll_table *
		      ...
		      int trickles_sock_poll_default (struct file *file,
						      struct socket *sock,
						      poll_table * wait);
		      extern void (*trickles_send_ack_hook) (struct sock *
							     sk);
		      ...
		      int trickles_setucont_default (int fd,
						     ...
						     unsigned ucont_len);
		      extern int (*trickles_getsockopt_hook) (struct sock *
							      sk, int level,
							      int optname,
							      char *optval,
							      int *optlen);
		      int trickles_getsockopt_default (struct sock *sk,
						       int level, int optname,
						       char *optval,
						       int *optlen);
		      extern void (*trickles_init_sock_hook) (struct sock *
							      sk, int val);
		      void trickles_init_sock_default (struct sock *sk,
						       int val);
			...
			struct pminisock *msk;
		      ...
		      static inline struct pminisock_key
			pminisock_toKey (struct cached_pminisock *cmsk)
		      {
			struct pminisock_key key;
			key.seq = cmsk->msk->seq;
			return key;
		      }
		      static inline unsigned pminisock_hash (struct
							     pminisock_key
							     *key)
		      {
			return hash ((u1 *) & key->seq, sizeof (key->seq), 0);
		      }
		      static inline int pminisock_cmp (struct pminisock_key
						       *k0,
						       struct cached_pminisock
						       *cmsk0,
						       struct pminisock_key
			...
			return k0->seq == k1->seq;
		      ...
		      static inline void pminisock_evict (struct
							  cached_pminisock
							  *cmsk)
		      {
			if (cmsk->msk == ((void *) 0))
			  ...
			  {
			    ;
			    msk_release (cmsk->msk->serverSK, cmsk->msk);
			  }
			cmsk->msk = ((void *) 0);
		      }
			...
			struct ParsedPMinisock_cell fastCell;
		      ...
		      {
			struct ParsedPMinisock_bucket buckets[(2048)];
			int count;
			int numBuckets;
			int maxCount;
			int hitCount;
			...
			for (i = 0; i < cache->numBuckets; i++)
			  {
			    ...
			    if ((*(int *) &(bucket)->fastCell.elem != 0))
			      {
				pminisock_evict (&bucket->fastCell.elem);
				ParsedPMinisock_deleteCell (&bucket->
							    fastCell);
			      }
			  }
		      }
			...
			*ParsedPMinisock_findInBucket (struct
			...
			return ((void *) 0);
		      ...
		      static struct ParsedPMinisock_cell
			*ParsedPMinisock_insert_helper (struct pminisock_key
							*key)
		      {
			struct ParsedPMinisock_cache_root *cache =
			  &ParsedPMinisock_cache;
			...
			return cell;
		      ...
		      static int ParsedPMinisock_insert (struct
							 cached_pminisock
							 *elem)
		      {
			;
			...
			return 1;
		      ...
		      static int ParsedPMinisock_find_helper (struct
							      pminisock_key
							      *key,
							      struct
							      ParsedPMinisock_cell
			...
			return found;
		      ...
		      static int ParsedPMinisock_find (struct pminisock_key
						       *key,
						       struct cached_pminisock
						       *result)
		      {
			...
			*(int *) &(bucket)->fastCell.elem = 0;
			...
			if (ParsedPMinisock_find_helper (&key, &cell))
			  {
			    struct pminisock *msk = cell->elem.msk;
			    ;
			    if (!(msk->saddr == iph->daddr &&
				  msk->source == th->dest &&
				  msk->daddr == iph->saddr &&
				  msk->dest == th->source))
			      {
			...
			return hash ((u1 *) key, sizeof (*key), 0);
		      ...
		      static inline int
			CachedWireContinuation_cache_cmp (unsigned *k0,
							  struct
							  CachedWireContinuation
							  *wcont0,
							  unsigned *k1,
			...
			return *k0 == *k1;
		      ...
		      static inline void CachedWireContinuation_evict (struct
								       CachedWireContinuation
								       *cwc)
		      {
		      }
			...
			char output[(16)];
		      ...
		      static inline NonceKey nonce_toKey (struct NonceMap
							  *map)
		      {
			return map->key;
		      }
			...
			return *k0 == *k1;
		      ...
		      static inline void nonce_evict (struct NonceMap *m)
		      {
		      }
			...
			struct Continuation_cell fastCell;
		      ...
		      {
			struct Continuation_bucket buckets[(2048)];
			int count;
			int numBuckets;
			int maxCount;
			int hitCount;
			...
			struct Continuation_cache_root *cache =
			  &Continuation_cache;
			return cell >=
			  (struct Continuation_cell *) cache->buckets
			  && cell <
			  (struct Continuation_cell *) (cache->buckets +
							(2048));
		      }
			...
			*Continuation_newCell (void);
		      ...
		      static void Continuation_evictOne (struct
							 Continuation_cell
							 **cell);
			...
			for (i = 0; i < cache->numBuckets; i++)
			  {
			    ...
			    if ((*(int *) &(bucket)->fastCell.elem != 0))
			      {
				CachedWireContinuation_evict (&bucket->
							      fastCell.elem);
			...
			*Continuation_insert_helper (CachedWireContinuationKey
			...
			if (!(*(int *) &(bucket)->fastCell.elem != 0))
			  ...
			  {
			    cell = &bucket->fastCell;
			    CachedWireContinuation_evict (&cell->elem);
			...
			return 1;
		      ...
		      static int
			Continuation_find_helper (CachedWireContinuationKey *
						  key,
						  struct Continuation_cell
						  **hint)
		      {
			...
			return found;
		      ...
		      static void Continuation_freeCell (struct
							 Continuation_cell
							 *cell);
		      static void Continuation_deleteCell (struct
							   Continuation_cell
			...
			Continuation_freeCell (cell);
		      ...
		      static int
			Continuation_delete (CachedWireContinuationKey * key)
		      {
			struct Continuation_cell *cell = ((void *) 0);
			int found = Continuation_find_helper (key, &cell);
			if ((found))
			...
			*(int *) &(bucket)->fastCell.elem = 0;
			...
			struct NonceKey_cell *cells;
		      ...
		      NonceKey_cache;
		      static inline int NonceKey_isFastCell (struct
							     NonceKey_cell
							     *cell)
		      {
			...
			return cell >= (struct NonceKey_cell *) cache->buckets
		      ...
		      static void NonceKey_evictOne (struct NonceKey_cell
						     **cell);
				...
				for (i = 0; i < cache->numBuckets; i++)
				  {
				    ...
				    if ((*(int *) &(bucket)->fastCell.elem !=
					 0))
				      {
					nonce_evict (&bucket->fastCell.elem);
					NonceKey_deleteCell (&bucket->
							     fastCell);
				      }
				  }
			      }
				  ...
				  return (0); 1;}
					...
					*NonceKey_insert_helper (__u64 * key)
					...
					return cell;
				      ...
				      static int NonceKey_insert (struct
								  NonceMap
								  *elem)
				      {
					if ((0) >= (2))
