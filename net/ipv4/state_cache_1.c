...
WireContinuation_copyIn(struct CachedWireContinuation *c,
                        ...
                        const struct cminisock *scont) {
        PseudoHeader *phdr = &c->hdr;
        phdr->seq = dcont->seq; phdr->type = dcont->continuationType; phdr->first = dcont->firstChild ? 1 : 0; phdr->serverAddr = scont->saddr; phdr->serverPort = scont->source; phdr->clientAddr = scont->daddr; phdr->clientPort = scont->dest;
...
WireContinuation_inOutComp(struct WireContinuation *w,
                           struct CachedWireContinuation *c) {
        return __builtin_memcmp(w->hmac_start, c->copy_start, (((__u8*)(((WireContinuation *)0) + 1) - (__u8*)((WireContinuation *)0)->hmac_start)));
}
...
void WireContinuation_insert(struct WireContinuation *wcont, const struct cminisock *scont) {
...
void dump_cache_stats(void) {
        printk("ParsedPMinisock" "\nLinksFollowed %d/%d Hitcount: %d / %d\nCell Memory %d/%d\n", (ParsedPMinisock_cache).linkCount, (ParsedPMinisock_cache).lookupCount, (ParsedPMinisock_cache).hitCount, (ParsedPMinisock_cache).total, ParsedPMinisock_cache.cellMem, ParsedPMinisock_cache.maxCellMem);
        printk("Continuation" "\nLinksFollowed %d/%d Hitcount: %d / %d\nCell Memory %d/%d\n", (Continuation_cache).linkCount, (Continuation_cache).lookupCount, (Continuation_cache).hitCount, (Continuation_cache).total, Continuation_cache.cellMem, Continuation_cache.maxCellMem);
}
