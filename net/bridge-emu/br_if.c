 ...
 *
 *	$Id: br_if.c,v 1.1.1.1 2004/06/19 05:03:04 ashieh Exp $
 *
...
#ifdef CONFIG_SMP
	// Not SMP-safe because we don't lock out the bottom halves
	// of other processors
#error "EMUSWITCH deallocation is not smp-safe"
#endif
	// Check for pending packets on queue
	int i;
	struct net_bridge_throttle_state *states[2] = {
		&p->input_state, 
		&p->output_state
	}, 
		*state;
	for(i=0; i < 2; i++) {
		...
		while(state->queue.qlen > 0) {
			struct sk_buff *skb = skb_dequeue(&state->queue);
			__kfree_skb(skb);
		}
	}
#endif
...
static struct net_bridge_port *new_nbp(struct net_bridge *br, struct net_device *dev)
{
	...
	p->priority = 0x80;
	INIT_NB_THROTTLE_STATE(&p->input_state);
	INIT_NB_THROTTLE_STATE(&p->output_state);
...
int br_add_if(struct net_bridge *br, struct net_device *dev)
{
	...
	if (dev->hard_start_xmit == br_dev_xmit) {
		printk("Warning: kernel allowing you to set bridge device as a bridge port. Possible forwarding loop\n");
#if 0
		return -ELOOP;
#endif
	}
